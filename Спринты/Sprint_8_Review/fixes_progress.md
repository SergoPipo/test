...
| S8R-AUDIT-014 | MEDIUM | ✅ | `assert [200, 200, 200, 200] == [200, 200, 200, 429]` | 10 backend + vitest | «не инкрементировать при fallback» → red | 3589/6xf/0; vitest 965; B после синхр.: 3623/5xf/0 | `9afbcb4` (wt B) | `1d92db59f28d` | /code-review не обязателен (ai/), — в ревью пакета |
...
## Следующий шаг
Цикл идёт, пакет **MEDIUM**. Ветка `s8r/fix-medium` (A = `wt-s8r-fixes`, вершина `9597055`, 3612/5xf/0, head миграций `6128b9c52d8a`), B = `wt-s8r-fixes-b` (detached; синхронизируется с вершиной A перед новой карточкой; коммиты B → cherry-pick в A).
Закрыто MEDIUM (24 из 51): 003, 004, 008, 009, 010, 011, 012, 013, 015, 021, 027, 029, 037, 038, 039, 040, 041, 056, 057, 058, 070, 071, 072, 076. В работе: 044 (A), 014 (B, возможна миграция).
Очередь A: 045 (cfg готов) → 046 → 047 → 062 → 063 → 006 → 077 → 094 → 096 → 095 (K2). Очередь B: 020 (cfg готов) → 054 → 051 → 053 → 065 → 066 → 097 → 081 → 082 → 073 → 083 → 084 → 085 → 086 → 087.
Правило гейтов: полный pytest оркестратора — с `-o faulthandler_timeout=240` и логом в файл, не в worktree, где идёт прогон DEV (S8R-FIX-024).
Вопросы заказчику (накоплено): торги выходного дня (093/072), stopped-сессии при удалении стратегии (069/015), block_shorts (032), порог SECRET_KEY (034), sandbox+real одним классом в дневном лимите (070).
HIGH: PR #29, S-1/S-2/S-7 — пн 28.09 с 10:00 МСК. BLOCKER: PR #28. Мерж — по команде заказчика.
