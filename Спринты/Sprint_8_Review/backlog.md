...
**Причина зависания — найдена этими же средствами и устранена (дефект продуктового кода).** Второй прогон с faulthandler'ом назвал точку: `tests/test_trading/test_ws_sessions.py::test_auth_via_cookie`, строка 143 — выход из `TestClient` (`thread.join()` → `_wait_for_tstate_lock`), портальный поток в `_cancel_all_tasks` → `run_forever` → `selectors.select`.

В `app/trading/ws_sessions.py` хендлер `trading_sessions_ws` в `finally` вызывал `task.cancel()` и сразу выходил. `cancel()` только помечает задачу к отмене — `CancelledError` доставляется на следующем тике loop, поэтому задачи оставались `pending`. Закрытие event loop идёт через `asyncio.runners._cancel_all_tasks`, который **ждёт их завершения**: ни таймаута, ни сообщения. Локально гонка почти всегда разрешалась в пользу быстрого завершения — отсюда «зелено локально, виснет в CI».

Это **не тестовая частность**: тот же cleanup отрабатывает в production при каждом отключении клиента от `/ws/trading-sessions/{user_id}`. Фикс — дожидаться отменённых задач через `asyncio.wait(pending, timeout=5)`; таймаут обязателен, иначе cleanup сам станет новым местом зависания.

Проверено: `test_ws_sessions.py` — 5 прогонов подряд по 6 passed; полный набор в конфигурации CI-шага (`DEBUG=false`, `--cov=app`, `faulthandler_timeout=120`) — **2223 passed / 1 xfailed / 0 failed за 3 мин 38 с**. Заведена [gotcha-48](../../Develop/stack_gotchas/gotcha-48-cancelled-tasks-not-awaited-hang-testclient.md), Stack Gotchas 47 → **48**.
