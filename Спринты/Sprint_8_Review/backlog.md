...
**Решение заказчику:** цель ТЗ выполняется до ~20 одновременных сессий. 40 — за пределом. Это не баг, а обнаруженный потолок; нужно ли поднимать его (батчинг записей, вынос `DailyStat` из горячего пути) — решение заказчика.
...
|---|---|---|
| `S8R-VOLUME-RUB-LOSES-LOT-SIZE` | low | При создании сделки `volume_rub = price × lots × lot_size` (`engine.py:1089`), но на fill в sandbox/real он **перезаписывается** как `fill_price × filled_lots` без множителя лота (`engine.py:1542`, `1601`, `PositionTracker.update_position`). Денежного эффекта нет — `cost_basis` из `volume_rub` используется только для paper, — но поле становится несопоставимым между режимами и врёт в аналитике/экспорте |
| `S8R-METRICS-STORE-SINGLE-PROCESS` | low | Буфер метрик живёт в памяти процесса: рестарт обнуляет графики, при нескольких воркерах каждый видит свою долю. Для однопроцессной поставки приемлемо; при масштабировании нужен внешний сток |
| `S8R-METRICS-JOBS-STILL-MOCK` | low | Пятый график `/admin/metrics` (backtest jobs) остался на mock-данных — источник у него не `@timed_event`, а таблица `backtest_jobs`, и читать её из WSGI-потока Dash нужно отдельным механизмом |
| `S8R-TELEGRAM-PER-COMMAND-METRIC` | low | ТЗ рисовал разбивку Telegram-latency по 5 командам, но `@timed_event("telegram.handle")` висит на обработчике update'а целиком и имени команды не видит. График сведён к общему p95 |
