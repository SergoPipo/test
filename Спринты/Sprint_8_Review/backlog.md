...
## ⬜ ОТКРЫТО на 2026-07-27 — что НЕ закрыто в этом цикле

| # | Что | Severity | Статус |
|---|-----|----------|--------|
| 1 | **Backend job CI красный на ВСЕХ ветках** — `pytest` падает на импорте: `RuntimeError: SECRET_KEY uses the insecure dev default with DEBUG=False`. CI не создаёт `.env` и не задаёт `DEBUG`/`SECRET_KEY`, а валидатор `check_production_secrets` (добавлен `84f04b2`, 2026-07-06) блокирует старт. Прогоны красные минимум с 2026-07-22 на `s8r/bug-31-unified-codegen`, `s8r/acceptance-fixes-2026-07-26`, `p1/be-trad-06`. **Следствие: backend-тесты в CI фактически не выполнялись ~3 недели.** Ранее это маскировалось ошибкой ruff на более раннем шаге — после её устранения дефект стал виден. | **high** (гейт качества не работает) | ⬜ не чинилось — требует правки `.github/workflows/ci.yml` (задать окружение для pytest-шагов) |
| 2 | 4 падающих E2E-теста в `s5-account.spec.ts` | low | ⬜ предсуществующие, вне объёма цикла |
| 3 | `auth-hardening.spec.ts` (7 тестов) не прогонялся на ветке доведения — захардкожены `localhost:8000` и `Origin: localhost:5173` | low | ⬜ требует штатной связки портов |
| 4 | E2E-набор виснет на длинных прогонах (`networkidle` + постоянный WS) | medium (мешает регрессии) | ⬜ обход есть (пофайлово), фикс спек — отдельная задача |
| 5 | `.env.example` не дополнен новыми `BACKTEST_DATA_TIMEOUT_MIN_SEC` / `MAX_SEC` / `PER_CHUNK_SEC` | low | ⬜ файл недоступен для правки в среде исполнителя |
| 6 | В основном worktree `Develop/` висит незакоммиченная правка `.env.example` (`AI_ALLOW_PRIVATE_PROVIDER_URLS`) — не из этого цикла | low | ⬜ требует решения заказчика |
| 7 | `S8R-SANDBOX-ACCOUNT-STALE-REOPEN` (см. ниже) | medium | ⬜ вне объёма цикла |
