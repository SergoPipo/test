...
## 📋 Остаток финальной приёмки 2026-07-26 (вердикт PASS WITH NOTES) — кандидаты в Sprint 9

> Все пункты ниже — **low/косметика**, ни один не блокировал Gate. Три находки той же приёмки (BUG-32 HIGH, FIND-06 MEDIUM, FIND-01 LOW) исправлены сразу в цикле — см. «Найденные баги» в `acceptance_checklist.md`.

| ID | Severity | Симптом | Где | Предложение |
|----|----------|---------|-----|-------------|
| **S9-ADMIN-METRICS-SPA-404** | low | SPA-роут `/admin/metrics` рендерит пустой `main` — ни графиков, ни 404/редиректа. Метрики открываются только прямой backend-ссылкой из AdminLandingPage. | `frontend/src/App.tsx` (`admin/*` → `AdminLayout`) | Либо честный редирект на backend-URL, либо страница-заглушка со ссылкой, либо 404. |
| **S9-BG-BACKTEST-503-UX** | low | Залповый запуск 4 фоновых бэктестов (клики в один тик) → backend 503 «База данных временно занята параллельной операцией», фронт показывает сырое «Ошибка: Request failed with status code 503». Плюс строка `backtests` успевает создаться до отказа → осиротевшие записи в `status='queued'` (наблюдались id 60–63). | `frontend` (обработка ошибок запуска), `backend/app/backtest/router.py::run_backtest_async` | Показывать `detail` из ответа; создавать строку `Backtest` **после** успешной постановки job (или удалять при отказе). |
| **S9-BACKTEST-DATA-TIMEOUT** | medium (проявилось только в условиях стенда) | Бэктест на периоде вне кэша при недоступном источнике данных зависает без таймаута: job `running` с progress=0 неопределённо долго, `backtests.status='queued'`, пользователю ни ошибки, ни прогресса; параллельные записи в БД начинают ждать лок. | `app/market_data/service.py::get_candles` (запрос к T-Invest), `app/backtest/jobs.py` | Таймаут на выборку данных + перевод job в `error` с внятным сообщением («не удалось получить свечи»); опционально — watchdog на job без прогресса. |
| **S9-AI-CHAT-UNSAVED-STRATEGY** | low | На `/strategies/new` поле AI-чата активно, но отправка молча проглатывается (`if (!strategyId) return`): текст исчезает без объяснения. | `frontend/src/components/ai/AIChat.tsx:218` | Блокировать поле с подсказкой «Сначала сохраните стратегию» либо авто-сохранять черновик перед отправкой. |
| **S9-BLOCK-PARSER-I18N** | low | Предупреждения парсера описания на английском («No indicators recognized in indicators section») при полностью русском UI. | `backend` (парсер описания → блоки) | Локализовать сообщения предупреждений. |
| **S9-EMAIL-TOGGLE-MISMATCH** | low | Тумблер Email включается для любого типа события, но письма шлются только для 5 из `EMAIL_ALLOWED_EVENTS`; для остальных настройка сохраняется, а письмо не уходит. | `backend/app/notification/email.py:26`, страница настроек уведомлений | Гасить/скрывать чекбокс для неподдерживаемых типов либо подписывать «email недоступен для этого события». |
| **S9-BOT-TOKEN-AUTOFILL** | low (UX/безопасность) | Поле «Bot token» в мастере (`type=password`, `autocomplete="off"`) всё равно автозаполняется менеджером паролей Chrome — в него подставился сохранённый пароль пользователя. | `frontend/src/components/wizard/FirstRunWizard.tsx` | `autoComplete="new-password"` + `data-lpignore="true"`, при желании — `type=text` с собственной маской. |
| **S9-DARK-DIMMED-CONTRAST** | low | Приглушённые подписи под статусами («GREEN (норма)», «RUNNING (4 jobs)») заметно тусклее основного текста — читаемо, но на грани. | `frontend` HealthWidget | Поднять на один шаг яркости в тёмной теме. |
