...
### Найдено попутно в этом цикле — НЕ чинилось (вне 8 карточек), требует решения

| Что | Где | Почему не чинили |
|-----|-----|------------------|
| `ruff check .` даёт `F401 sqlalchemy imported but unused` | `backend/alembic/versions/c9f1a2b3d4e5_uq_user_notif_settings.py:26` | Предсуществующая ошибка на базовой ветке (зафиксирована в baseline до правок). CI гоняет `ruff check .` из `backend/` → job `lint` красный. Фикс — удаление одной строки. |
| Экспорт PDF отдаёт 503 с английским техническим текстом `WeasyPrint not installed` | `backend/app/backtest/router.py:1066` (`detail=str(e)`) | Под контракт C-1 не подпадает, но пользователю показывать нечего. |
| Сырой `e.message` при применении параметров к стратегии | `frontend/src/components/backtest/GridSearchHeatmap.tsx:376` (`applyParamsToStrategy`) | Это не запуск бэктеста — вне карточки S9-BG-BACKTEST-503-UX. |
| Карточка фонового бэктеста «висит» в `running` после F5, если job упал при закрытой вкладке | `useBackgroundBacktestsBootstrap` | Нужна REST-реконсиляция через `backtestApi.listJobs()` на bootstrap + расширение типа `getJob()` полем `error_message`. Крупнее косметики — отдельный тикет. |
| `PUT /notifications/settings/{event_type}` принимает `email_enabled=true` для неподдерживаемого типа | `backend/app/notification/router.py` | Защита сейчас только на UI. Серверный 422 сломал бы wizard-flow и существующий контракт; нужна либо тихая нормализация, либо решение заказчика. |
| Плавающее падение `test_trading/test_order_manager.py::test_max_positions_limit` | backend тесты | Упало 1 раз из 3 полных прогонов сведённой ветки; изолированно и в 2 других прогонах — зелёное. `app/trading/` в этом цикле не менялся. DEV-4 независимо наблюдал зависание полного прогона под параллельной нагрузкой. Похоже на чувствительность к нагрузке/порядку — нужен отдельный разбор. |
