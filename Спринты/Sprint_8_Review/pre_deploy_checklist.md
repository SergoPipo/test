...
| Карточка | Sev | Суть | Статус |
|---|---|---|---|
| `S8R-RESUME-STOP-NO-SESSION-LOCK` | low | «Стоп» + «Возобновить» одновременно — побеждает последний коммит | ✅ цикл `s8r/pre-deploy` |
| `S8R-ENTRY-PATH-NO-CLOSE-LOCK` | low | путь открытия позиции не берёт лок сделки | ✅ цикл `s8r/pre-deploy` |
| `S8R-FIGI-NO-NEGATIVE-CACHE` | low | неудачный резолв FIGI не кэшируется, запрос повторяется на каждый сигнал | ✅ цикл `s8r/hygiene`, TTL 15 мин |
| `S8R-PAPER-TARIFF-UNREACHABLE-FROM-UI` | low | наследование тарифа счёта paper-сессией через UI недостижимо | ✅ цикл `s8r/hygiene` |
| `S7R-E2E-7.9-MISSING` | low | нет E2E на backup/restore CLI — единственный непокрытый пункт плана S7 | ✅ цикл `s8r/hygiene`, pytest (Q7=b) |
| `S7R-BG-BACKTEST-AUTOCOLLAPSE` | low | завершённые фоновые бэктесты не сворачиваются сами | ✅ цикл `s8r/hygiene`, 30 с |

**Отдельно — решение, а не работа:**

- `S8R-TRADES-WITHOUT-FIGI-UNMATCHABLE` (low) — на вашем стенде таких сделок
  **нет** (все sandbox-сделки закрыты). ✅ закрыта как неприменимая (Q8=a
  цикла `s8r/pre-deploy`).
- `S5R-BLOCKLY-MODE-B-MODAL` + `S5R-BLOCKLY-MODE-B-CHECK` (low) — два
  `test.skip` в `e2e/blockly.spec.ts:86,90` на функцию «mode B» (шаблоны),
  которая не реализована с S5. ✅ функция сделана в цикле `s8r/hygiene`
  (кнопка в редакторе, шаблон приведён к 7 секциям парсера), оба спека сняты
  со skip и проходят.
...
**Что осталось из всего чеклиста:** только `S8R-W5-DOCKER-COMPOSE-VALIDATE`
из раздела A (docker на машине разработки отсутствует, стек ни разу не
запускался — риск зафиксирован осознанно, Q3=b) и наблюдения раздела C,
проверяемые только после развёртывания.

**Раздел C, уточнение от 2026-08-24.** Из трёх наблюдений одно частично
проверено браузером: **визуальный resize фигур работает** — захват за угол
двигает одну точку, противоположная остаётся, результат сохраняется. Три
смежных сценария воспроизвелись неустойчиво и вынесены в
`S9-CHART-DRAWING-EDIT-OBSERVATIONS` для ручной проверки. **Живые p50/p95
«сигнал → ордер» по-прежнему недоступны без прод-стенда под нагрузкой** —
локально повторяется только synthetic-замер, уже имеющийся в
`Sprint_8/perf_baseline_w5.md`. `S8R-TINVEST-SANDBOX-FLAKY-70001` из этого
раздела закрыта.
