...
- **AUTH-HARDENING мини-волна (backend+frontend, координированно)** — объединяет `P1W2-REFRESH-GRACE` (grace-окно ротации + общий single-flight + cross-tab sync → устраняет принудительный разлогин при двух вкладках/AI-чате) и **CFG-FE-01** (токены `localStorage → HttpOnly cookie`), вынесенный из Волны 3. Решение заказчика (2026-07-07): делать отдельной координированной волной, т.к. модель хранения токенов нельзя менять «наполовину». Детали и скоуп — `P1_AUTH_HARDENING_HANDOFF.md`.
