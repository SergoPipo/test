...
- База: **`p1/wave2-backend` @ `9cd44aa`** (Develop, moex-terminal). В ней уже: атомарный `_revoke_jti` (ON CONFLICT), CSRF-exempt `/auth/refresh`, ротация refresh, `_set_access_token_cookie` для access-токена (частично cookie-based — см. `backend/app/auth/router.py`).
...
## Когда делать
Порядок с Волной 3 — на усмотрение заказчика: можно до, после или параллельно (файлы `auth/*` и `authStore/client/aiStreamClient` пересекаются с `fix/fe-security` CFG-FE-02 и `fix/fe-network` FE-NET-02 — при параллельной работе координировать мерж `authStore.ts`/`useWebSocket.ts`).
