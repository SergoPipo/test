...
```
ТЕКУЩЕЕ ДЕЙСТВИЕ (2026-08-06): цикл «FIGI сделок, гонка реконсиляций и
  ...
  Sprint 9 по-прежнему НЕ стартовать.
    ...
    - S8R-ENV-KEYS-TOO-SHORT (medium) — ключи в вашем .env короче нормы:
      ENCRYPTION_KEY 31 байт (нужно ≥32), SECRET_KEY 23 байта. Содержимое
      .env не читалось, длины видны из предупреждений. Смена ENCRYPTION_KEY
      потребует заново ввести брокерские токены — решение за вами.
