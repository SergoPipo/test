...
>
> Последнее обновление: 2026-07-29 (**closeout S8R: backend job в CI снова работает — тесты не выполнялись ~3 недели; E2E-набор идёт одним прогоном**. Первый реальный прогон CI вскрыл и закрыл три дефекта, включая гонку в WS-хендлере, вешавшую весь набор. Плюс закрыт `S8R-ALEMBIC-FRESH-DB-DRIFT` — чистая установка отдавала 500 на входе. ТЗ → v1.8.)
>
...
> **Первый за три недели реальный прогон backend-job в CI вскрыл три дефекта, невидимых локально.** (а) `test_auth_cookie_secure` — 3 теста держались на `DEBUG=true` из `.env` разработчика: при `DEBUG=False` cookie уходят с `Secure`, httpx их по http не переотправляет → 403. (б) `test_backup_cli` — 2 теста: подпроцесс стартует со стерильным окружением и держался на наличии `.env`. (в) **`app/trading/ws_sessions.py` — гонка в продуктовом коде:** в `finally` задачи только помечались `cancel()`, никто не дожидался завершения, и при закрытии event loop `asyncio._cancel_all_tasks` ждал их вечно — прогон вис намертво, job горел до 6-часового лимита runner'а. Тот же cleanup отрабатывает в production при каждом отключении клиента от `/ws/trading-sessions/{user_id}`. Диагностику дали добавленные в `ci.yml` `timeout-minutes: 40`, `PYTHONUNBUFFERED=1` и `faulthandler_timeout=120`. Stack Gotchas 47 → **48** ([gotcha-48](../Develop/stack_gotchas/gotcha-48-cancelled-tasks-not-awaited-hang-testclient.md)). Урок: сломанный гейт не просто не ловит новые дефекты — он копит старые.
> Гейты closeout: pytest **2223 / 1 xfailed / 0 failed** (в конфигурации CI-шага: `DEBUG=false`, без `.env`, с coverage), vitest **832 / 120 файлов**, tsc 0, eslint 0, ruff 0, mypy 0, bandit 0 medium+. ТЗ §9 дополнено контрактом окружения CI, запретом `networkidle` в спеках и требованиями к стенду `auth-hardening` (ТЗ **v1.7 → v1.8**). ФТ не менялись — пользовательское поведение не затронуто.
>
...
## Текущая фаза: **Gate Sprint_8_Review пройден (PASS WITH NOTES, 2026-07-26; чеклист дозакрыт 2026-07-27) → ожидание мержа PR #9, затем старт Sprint 9 «Перевод в продуктив»**
...
```
ТЕКУЩЕЕ ДЕЙСТВИЕ (2026-07-27): Gate Sprint_8_Review ПРОЙДЕН — PASS WITH NOTES,
  остаток приёмки (8 замечаний) ЗАКРЫТ в тот же день. Дозакрыт последний пункт
  чеклиста S8.8 (BUG-33/BUG-34 — фигуры разметки исчезали с графика).
  Непокрытых пунктов приёмки не осталось.

  ОЖИДАЕТ РЕШЕНИЯ ЗАКАЗЧИКА:
    - PR #9 (s8r/acceptance-fixes-2026-07-26 → s8r/bug-31-unified-codegen) — 5 фиксов
      приёмки (BUG-32/33/34, FIND-06, FIND-01). Пока не смёржен, прод-кандидат их не содержит.
    - Коммит + PR по ветке s8r/backlog-fixes-2026-07-27 (8 карточек остатка). Изменения
      сведены и проверены, но НЕ закоммичены — ждут команды. PR должен идти в
      s8r/bug-31-unified-codegen ПОСЛЕ мержа PR #9, иначе конфликт по базе.
    - 6 находок «попутно», сознательно не чиненных (список — Sprint_8_Review/backlog.md,
      раздел «Найдено попутно»). Среди них ruff F401 в alembic-миграции, из-за которого
      CI job `lint` красный на базовой ветке.
    - Старт Sprint 9 «Перевод в продуктив» (заказчик: «пока не стартуем»).
    - Судьба стенда s8r-acceptance (оставлен поднятым для дальнейших тестов).

  Взять в Sprint 9 из приёмки: живые p50/p95 «сигнал→ордер» под нагрузкой, повтор S8.7
  «реальная сделка → 3 канала» в торговые часы, разовый drain paper-сессий при выкатке
  (deployment_guide §7). Косметика S9-backlog (8 карточек) в S9 НЕ переносится —
  закрыта 2026-07-27 в цикле приёмки.
