...
ТЕКУЩЕЕ ДЕЙСТВИЕ (2026-08-06): цикл «один инструмент — одна сессия на
  ...
  Q7=a, Q8=a, Q9, Q10, Q11). Q12 остался без ответа — решение принято
  исполнителем и вынесено в backlog отдельным пунктом.
  Sprint 9 по-прежнему НЕ стартовать.
    ...
    - S8R-PAPER-CONFLICT-BY-VERSION-NOT-TIMEFRAME (low) — две paper-сессии по
      одному тикеру на разных таймфреймах от одной версии стратегии запустить
      нельзя, хотя сценарий штатный. Не трогалось: Q1=a про добавление
      ограничений, а не про снятие существующих.
