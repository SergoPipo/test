...
>
> Последнее обновление: 2026-09-23 (**S8R, аудит «с нуля» A–T: 101 карточка-рецепт, 3 блокера деплоя (побег из песочницы, инъекция через SOURCE, открытый /auth/setup), ничего не чинилось — по решению заказчика Q1**. Отчёт `Sprint_8_Review/audit_2026-09.md`, задание исполнителю `prompt_s8r_audit_fixes.md`, PR moex-terminal #27 с 26 доказательными xfail-тестами. Гейты = baseline.)
>
> **2026-09-23 — S8R: независимый аудит с нуля (17–23.09, оркестратор + 18 ревьюеров, ветка кода `s8r/audit` от `develop` `eb769cc`).** Аспекты A–T по промпту заказчика; **ни одна находка не исправлялась** (Q1) — 101 карточка `S8R-AUDIT-001…101` (3 blocker / 24 high / 53 medium / 21 low) с рецептами для junior-модели и 26 `xfail(strict=True)`/`it.fails`-тестов, зелёных в CI до фикса. Блокеры: код стратегии выходит из песочницы (001, воспроизведено), `SOURCE` конструктора попадает в код без экранирования (002), `POST /auth/setup` открыт всегда (033). HIGH — торговый контур («нет ответа ≠ нет ордера» 024/007/030/074, частичное исполнение 025, сайзинг 026, стрим 061, CB просадки 068, short 032, SL/TP 075, удаление стратегии 069, риск-параметры 080), секреты/сессии (034–036), фронт `localhost` (055), бэктест-пул (078), корп. действия (089–091), backup (092), календарь MOEX (093), гонки счетов (099/101). Пройдены полностью A, B, C, D, E, G, J, K, P, Q, R, T; частично F, H, I, L, M, N, O; **S (живой брокер) не проходился** — ни одного ордера на счёте #3. Ждут решений заказчика: 001, 033, 032, 027, 024/030, 081, 070, 087, 091, 095, G2-09, 2FA, GPL, 049, `stash drop`. Sprint 9 не стартовал.
>
...
## Текущая фаза: **Sprint_8_Review — после аудита «с нуля» (2026-09-23): 101 карточка открыта, 3 блокера деплоя; следующий цикл — фиксы по `prompt_s8r_audit_fixes.md` после решений заказчика. Sprint 9 не стартовать.**
...
```
ТЕКУЩЕЕ ДЕЙСТВИЕ (2026-09-25): цикл фиксов аудита — ИДЁТ. Пакет HIGH закрыт
  по коду: 25/25, PR moex-terminal #29 (стек на #28), E2E 172/0, pytest 3035,
  покрытие 88 %, три оси ревью чисто; S-1/S-2/S-7 на счёте #3 — 28.09 с 10:00
  МСК. Следующий — пакет MEDIUM (52 карточки). Мерж #28/#29 — по команде.
  Продолжение: «выполни @Спринты/Sprint_8_Review/prompt_s8r_fixes.md —
  продолжение»; точное состояние — fixes_progress.md, «Следующий шаг».
  Промпт prompt_s8r_fixes.md (локальный); ответы заказчика, среда и статус
  каждой карточки — Sprint_8_Review/fixes_progress.md (читать первым при
  продолжении). Объём Q1=d: BLOCKER → HIGH → MEDIUM → LOW, ветка на пакет
  (s8r/fix-blocker|high|medium|low), PR после гейтов, мерж — по команде
  заказчика. DEV-субагенты ≤ 2 параллельно (Q8=b), живой прогон S-1/S-2/S-7
  на счёте #3 после HIGH (Q7=a), дозакрытие аспектов F/H1/I2 (Q11=b).
  PR #27 смёржен в develop → 7ddb970. Worktree кода:
  /Users/sergopipo/Documents/Claude_Code/wt-s8r-fixes (+ -b для второго DEV).
  Q12: logs/dev.log проверен счётчиками — токена брокера нет.

  ПАКЕТ BLOCKER — ЗАКРЫТ, PR moex-terminal #28 (s8r/fix-blocker → develop),
  мерж — по команде заказчика:
    033 (967fe10) /auth/setup закрыт после первого пользователя, новые — admin;
    002 (47bf073) значения полей блоков по схеме, SOURCE не интерполируется;
    001 (c6bf111) исполняется только код из IR блоков, прокси вместо модулей,
        legacy live-путь и /sandbox/* удалены (попутно закрыта 028).
    Гейт: 2711 passed / 19 xfailed / 0 failed, coverage 87 %, E2E 171/1/0;
    три оси ревью: 0 / 0 / 1 low (исправлено 9f676aa).
  ПАКЕТ HIGH — НА ПАУЗЕ (ветка s8r/fix-high стеком от s8r/fix-blocker,
    запушена до 6012089): закрыты 034, 035 (миграция c5e8b2a7f913), 036,
    093, 007, 089; 090 закоммичен в worktree B (6e5a6c2), не перенесён;
    024 и 091 прерваны посреди работы (частичные правки не закоммичены). ВОПРОС ЗАКАЗЧИКУ (093): торги выходного дня MOEX
    (с 2025) — считать ли Сб/Вс торговыми днями терминала? Сейчас — нет.
  Sprint 9 НЕ стартовать.

ПРЕДЫДУЩЕЕ ДЕЙСТВИЕ (2026-09-23): аудит «с нуля» — ВЫПОЛНЕН, фиксы НЕ начаты
  Ветка кода s8r/audit от develop eb769cc — только доказательные тесты,
  PR moex-terminal #27 → develop (CI зелёный). Кода приложения не касались.
  Отчёт: Sprint_8_Review/audit_2026-09.md (101 карточка, §1–§9).
  Задание исполнителю: Sprint_8_Review/prompt_s8r_audit_fixes.md.
  Прогресс/чекпойнты: Sprint_8_Review/audit_progress.md.

  ЧТО ДАЛЬШЕ (по команде заказчика):
    1. Решения заказчика по списку из audit_2026-09.md §8
       (001, 033, 032, 027, 024/030, 081, 070, 087, 091, 095, G2-09).
    2. Мерж PR #27 в develop; цикл фиксов по prompt_s8r_audit_fixes.md:
       BLOCKER 033 → 002 → 001, затем HIGH, MEDIUM, LOW — каждая карточка
       начинается со снятия xfail-маркера (RED) и заканчивается GREEN.
    3. После пакета HIGH — живой прогон S-1/S-2/S-7 на счёте #3.
    4. Дозакрытие аспектов ⏸ (F, H1, I2, L2–L7, M, N, O, S) — отдельно,
       если заказчик решит.
    5. Заказчику руками: git stash drop stash@{0} в Develop/ (после
       проверки .env.example); команды удаления merged-веток из карточки 049;
       ручная проверка трёх сценариев рисования мышью.

  Уборка после аудита выполнена: стенд :8000/:5173 остановлен, worktree
  wt-s8r-audit удалён, Vite-кэш снят, симлинк .env не создавался,
  рабочая БД и счёт #3 не затрагивались, Develop/ на develop eb769cc
  (правка .env.example заказчика сохранена).

ПРЕДЫДУЩЕЕ ДЕЙСТВИЕ (2026-08-10, вечер): цикл «гигиена раздела B»
  ВЫПОЛНЕН ПОЛНОСТЬЮ — ветка s8r/hygiene от develop 7827745.
  Ответы заказчика: Q1=a, Q2=a, Q3=a, Q4=a, Q5=a, Q6=b, Q7=b, Q8=a,
  Q9=a, Q10 — ветка s8r/hygiene, пуш, PR, свод документации в main.
  Sprint 9 по-прежнему НЕ стартовать: S9 = только развитие, после сдачи.

    РАЗДЕЛ B ЧЕКЛИСТА ЗАКРЫТ ЦЕЛИКОМ — пять карточек, которые
    прошлый цикл не успел (время ушло на сломанную сборку фронтенда):

    1. Режим B снова доступен. Импорт стратегии из текстового шаблона
       жил в коде с Sprint 5, но кнопки не было — функция была
       недостижима, а два E2E-теста стояли отключёнными и создавали
       ложное впечатление покрытия. Кнопка «Режим B» вернулась в шапку
       редактора стратегии. Сам шаблон исправлен: было десять секций,
       разбирается семь — название, инструменты и таймфрейм
       пользователь заполнял впустую. Добавлена кнопка «Скопировать
       пример»: по пустым заголовкам внешний ИИ пишет текст, который
       парсер не понимает. Оба теста сняты с отключения и проходят.
    2. Тариф брокерского счёта для бумажной сессии стал достижим из
       формы запуска. Наследование работало на сервере с 04.08, но
       форма отправляла счёт только для песочницы и реального режима —
       ветка не включалась ни при каком действии пользователя. Теперь
       в форме выбор: ввести ставку вручную или взять тариф счёта;
       выбранный тариф виден сразу, нулевой сопровождается
       предупреждением. Ставка фиксируется при запуске.
    3. Завершённые фоновые бэктесты сворачиваются сами через 30 секунд.
       Карточка с ошибкой остаётся — причина отказа больше нигде не
       видна; при открытом списке отсчёт не идёт.
    4. Терминал перестал долбить брокера из-за одного нерезолвящегося
       тикера: отрицательный ответ помнится 15 минут, а сбой связи НЕ
       кэшируется — иначе минутная потеря сети ослепила бы терминал на
       четверть часа уже после восстановления.
    5. Сценарий 7.9 (backup/restore) закрыт pytest-тестом: проверяются
       данные, а не коды возврата — восстановление потерянной строки,
       целостность файла, содержимое pre-restore-снимка, попадание в
       бэкап транзакции из WAL, ротация по времени.

    Миграций нет. Живой sandbox-счёт заказчика не затрагивался.
    Документы: ФТ v3.9, ТЗ v2.9, ui_checklist_s8 — 267 пунктов
    (секции S8.35–S8.38).

  Предыдущий цикл (доведение до деплоя, ветка s8r/pre-deploy):
  Ответы заказчика: Q1=c, Q2=b, Q3=b, Q4=b, Q5=b, Q6=a, Q7=b, Q8=a,
  Q9=a, Q10=b, Q11=a, Q12 — ветка s8r/pre-deploy, пуш и PR.
  Sprint 9 по-прежнему НЕ стартовать.

    ГЛАВНОЕ: PRODUCTION-СБОРКА ФРОНТЕНДА НА develop БЫЛА СЛОМАНА.
    frontend/Dockerfile собирает образ командой `pnpm build`
    (= `tsc -b && vite build`), а `tsc -b` падал со 102 ошибками типов в
    45 файлах. То есть первый же шаг развёртывания — `docker compose build`
    — завершился бы ошибкой, и об этом мы узнали бы на Mac mini.

    ПОЧЕМУ ЭТОГО НИКТО НЕ ВИДЕЛ: гейт «tsc 0», зелёный во всех прошлых
    циклах и стоящий в CI (`pnpm tsc --noEmit`), НЕ ПРОВЕРЯЛ НИ ОДНОГО
    ФАЙЛА. Корневой frontend/tsconfig.json — solution-файл с "files": [],
    и без флага -b такая команда завершается мгновенно с кодом 0.

    Среди 102 ошибок были настоящие дефекты, а не только расхождения
    в тестовых фикстурах:
    1. Тип сделки объявлял направление как long/short, backend отдаёт
       buy/sell — сравнения в UI компилятор считал заведомо ложными.
    2. После апгрейда библиотеки сплит-панелей проп переименован
       (direction → orientation): в трёх местах он молча игнорировался.
    3. Страница списка бэктестов читала поля, которых нет в её типе.
    4. Запуск бэктеста мог вернуть undefined вместо id, и подписка на
       прогресс вешалась в пустоту.
    Все 102 устранены, `pnpm build` проходит, dist/ собирается.
    Гейт починен: в package.json добавлен `typecheck: tsc -b`, в CI
    шаг заменён на `pnpm typecheck` + добавлен `pnpm build` — ровно то,
    что делает Dockerfile.

    ЧТО ЕЩЁ ИЗМЕНИЛОСЬ ДЛЯ ПОЛЬЗОВАТЕЛЯ:
    1. «Запустить заново» снова работает для песочницы и реального счёта
       (раньше давала 422: терялись счёт, таймфрейм и сам режим). Тело
       запроса теперь собирает одна общая функция — та же, что у формы
       запуска, поэтому поля больше не разъедутся.
    2. Кнопка «Запустить заново» гасится заранее, если по этому
       инструменту на этом счёте уже работает другая сессия, — с текстом,
       какая именно мешает.
    3. Брокерский счёт нельзя удалить, пока на нём есть работающие,
       приостановленные или снятые рестартом сессии: раньше удаление
       молча обнуляло у них привязку к счёту, и они продолжали торговать
       вне запрета дублей. В отказе перечислены мешающие сессии.
    4. Курс доллара в виджете баланса — по официальному фиксингу ЦБ РФ
       (было: константа 90 ₽/$). Если ЦБ недоступен — показывается
       последний известный курс с датой; если неизвестен вовсе —
       переключатель валюты просто не показывается.
    5. Двойной клик «Стоп» одновременно с «Возобновить» больше не может
       тихо отменить остановку.

    КОД-РЕВЬЮ (4 независимых ревьюера): 4 находки, все закрыты здесь же.
    Главная — про мою же правку: лок сделки на пути открытия позиции давал
    встречную блокировку двух сессий одного sandbox-счёта (обе простояли бы
    до 30 секунд, а сверка после переоткрытия сорвалась бы). Восстановление
    теперь идёт без удержания этого лока. Ещё три: bandit-подавления не были
    занесены в .bandit; отказ сервера при «Запустить заново» пропадал молча;
    правка типов теряла вложенное условие в старом формате блоков.

    ГЕЙТЫ: pytest 2589 / 1 xfailed / 0 failed · vitest 902 / 129 файлов ·
    tsc -b 0 (впервые по-настоящему) · pnpm build ✅ · eslint 0 ·
    ruff 0 · mypy Success (178) · bandit 0 medium+.
    МИГРАЦИЙ НЕТ (Q9=a), alembic head прежний — a7b8c9d0e1f2.
    РАБОЧАЯ БД НЕ ТРОГАЛАСЬ, живой sandbox-счёт #3 не открывался (Q10=b).

    НЕ СДЕЛАНО (осознанно, приоритет отдан сломанной сборке):
    пять карточек гигиены раздела B — негативный кэш FIGI, тариф paper
    из UI, E2E на backup/restore CLI, авто-сворачивание фоновых
    бэктестов и возврат «Режима B» (Q7=b). Остаются открытыми
    в backlog с оценками.

  ---
  Предыдущее (2026-08-06): цикл «один инструмент — одна сессия на
