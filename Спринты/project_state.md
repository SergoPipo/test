...
>
> Последнее обновление: 2026-07-30 (**хвост S8R: закрыты все 5 карточек backlog блока A, пройдены 3 из 4 прод-проверок, метрики стали живыми**. Гейты: pytest 2276, vitest 842, E2E 162/3/0. Новая ловушка — gotcha-50. ТЗ → v1.9.)
>
...
ТЕКУЩЕЕ ДЕЙСТВИЕ (2026-07-29): Sprint_8_Review закрыт содержательно.
  ...
  ОЖИДАЕТ ДЕЙСТВИЯ ЗАКАЗЧИКА:
    - КОММИТ И ВЕТКИ. Работа хвоста лежит НЕЗАКОММИЧЕННОЙ в worktree
      Develop/.claude/worktrees/s8r-tail (ветка s8r/tail-fixes от develop
      81ef068). Правки документации — в рабочем дереве репо Test.
      Нужны имена веток отдельно для каждого из двух репозиториев.
    - B2 — «реальная сделка → уведомление в 3 канала». Нужен пароль
      пользователя sergopipo и работающий стенд с T-Invest/Telegram.
      MOEX торгует 10:00–18:45 МСК.
    ...
    - Решение по потолку производительности: цель ТЗ «signal→order p95
      < 500 мс» держится до ~20 одновременных сессий, при 40 нарушается
      в 1,7×. Поднимать ли потолок — решение заказчика.
