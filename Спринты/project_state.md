...
ТЕКУЩЕЕ ДЕЙСТВИЕ (2026-07-29): Sprint_8_Review закрыт содержательно.
  ...
  ТЕКУЩЕЕ ДЕЙСТВИЕ: следующий цикл — починка находок части 2
  (см. prompt_s8r_findings.md). Sprint 9 по-прежнему НЕ стартовать.
    - Решения по новым находкам (см. backlog, раздел «Хвост S8R, часть 2»):
      формулировка пункта приёмки S8.7 (email не для событий сделок);
      пересчитывать ли исторический PnL sandbox-сделок с 2026-07-06
      (фикс D1 не ретроактивен); realized_pnl при закрытии через
      close_position; мёртвый код PositionTracker.
