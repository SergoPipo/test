...
## S8.28 Сверка позиций с брокером и доучёт закрывающего ордера (S8R, 2026-08-05)

Добавлено по итогам `S8R-EXIT-ORDER-NOT-TRACKED` (Q1=a) и
`S8R-SANDBOX-POSITIONS-NOT-RECONCILED` (Q2=a).

- [ ] Карточка сессии с расхождением показывает красный бейдж «Расхождение с брокером» рядом со статусом.
- [ ] В подсказке бейджа видны **оба** числа — сколько бумаг у брокера и сколько в терминале, — и номер сделки.
- [ ] Сессия с расхождением своих позиций переведена в «Пауза», а сама сделка в журнале осталась открытой: `exit_price` и P&L пусты, статус не «Закрыта».
- [ ] Уведомление о расхождении приходит один раз, а не на каждом перезапуске приложения при том же самом расхождении.
- [ ] Когда позиции сошлись, после следующей сверки бейдж пропадает сам.
- [ ] Позиция, о которой знает брокер, но не знает терминал, показывается с оговоркой «возможно, куплено вручную» и сессию не останавливает.
- [ ] После перезапуска backend сделка, чей закрывающий ордер исполнился у брокера, доезжает до «Закрыта» с фактической ценой и комиссией (не с ценой уровня стопа).
- [ ] Такая сделка учтена в дневной статистике ровно один раз (число закрытых не задвоилось).
- [ ] Пока закрывающий ордер в полёте, кнопка «Закрыть» отвечает понятным «Ордер на закрытие уже отправлен брокеру», а второй ордер не уходит.
- [ ] При сбое песочницы пользователь видит «Песочница брокера временно недоступна, попробуйте ещё раз», а не код gRPC.
- [ ] При сбое на отправке ордера сообщение дополнительно предупреждает сверить портфель у брокера — ордер мог уйти.

## S8.29 Настоящий FIGI и фоновая сверка позиций (S8R, 2026-08-06)

Добавлено по итогам `S8R-SANDBOX-TRADES-FAKE-FIGI` (Q1=a),
`S8R-RECONCILE-LOCK-DIVERGENCE` (Q2=b) и `S8R-RECONCILE-NOT-PERIODIC` (Q3=a).

- [ ] Позиция, открытая стратегией в песочнице или на реальном счёте, при сверке **сопоставляется** с портфелем брокера: сессия остаётся активной, бейджа «Расхождение с брокером» нет.
- [ ] Бумажный режим не изменился: paper-сделки открываются и закрываются как раньше, сверка их не касается.
- [ ] Расхождение обнаруживается **в течение дня**, а не только при запуске приложения: закрыв позицию вручную в приложении брокера, бейдж видно в течение ближайших 15 минут без перезапуска терминала.
- [ ] Разобравшись с расхождением, бейдж исчезает сам после очередной сверки — перезапуск терминала не нужен.
- [ ] Вторая сессия на том же брокерском счёте (по **другому** тикеру) не останавливает первую: обе продолжают торговать. По одному тикеру две сессии на счёте с 2026-08-06 запрещены — см. S8.30.
- [ ] Ручная докупка той же бумаги на счёте стратегии не ставит сессию на паузу — расхождение показывается, торговля продолжается.
- [ ] Нехватка бумаг у брокера по-прежнему останавливает сессию: это единственный случай, когда стратегия торгует несуществующей позицией.
- [ ] Сессия, возобновлённая после расхождения и снова ушедшая на паузу, сопровождается уведомлением — «Возобновить» не выглядит нерабочей кнопкой.
- [ ] Пока идёт закрытие позиции, сессия не уходит на паузу с сообщением «у брокера позиции нет» — встречный ордер в полёте расхождением не считается.
- [ ] Недоступность брокера во время фоновой сверки не подвешивает приложение: графики продолжают обновляться, уведомления приходят.

## S8.30 Один инструмент — одна сессия на счёте (S8R, 2026-08-06)

Добавлено по итогам `S8R-DUPLICATE-SESSION-PER-INSTRUMENT` (Q1=a, Q2=a, Q3=a,
Q4=a, Q6=a) и `S8R-RECONCILE-NO-ACCOUNT-DEDUP`.

- [ ] Вторую сессию по тому же тикеру на том же брокерском счёте запустить нельзя: кнопка «Запустить» неактивна, а под формой видно объяснение.
- [ ] Текст отказа называет инструмент, счёт и **номер** мешающей сессии, а не «ошибка 422» и не пустой экран.
- [ ] Тот же тикер на **другом** счёте запускается свободно — запрет не шире, чем нужно.
- [ ] Другой тикер на том же счёте запускается свободно.
- [ ] Разные таймфреймы запрет не обходят: пул бумаг у брокера общий независимо от таймфрейма.
- [ ] Бумажный режим не ограничен: две paper-сессии по одному тикеру запускаются свободно — и на разных таймфреймах, и на одинаковых настройках, в том числе от одной версии стратегии.
- [ ] У каждой такой paper-сессии свой портфель: сделки и P&L не смешиваются между ними.
- [ ] «Возобновить» у сессии на паузе отказывает, если по её инструменту на счёте уже работает другая, — и объясняет, какая именно.
- [ ] Остановленная (`stopped`) сессия запуску новой по тому же инструменту не мешает: история сделок не блокирует торговлю.
- [ ] После перезапуска терминала пара конфликтующих сессий в истории не ломает восстановление: поднимается торговавшая до остановки (активная, затем приостановленная — раньше стоявшей на паузе); вторая, если победитель поднялся, переводится на паузу с уведомлением «Пара счёт+тикер занята сессией #N», иначе остаётся «приостановлена» и повторяется при следующем запуске (S8R-AUDIT-010).
- [ ] Сверка с брокером не показывает ложного расхождения, когда бумаги счёта числятся за несколькими сессиями: сравнивается портфель счёта с суммой по счёту.

## S8.31 «Запустить заново» для песочницы и реального счёта (S8R, 2026-08-10)

- [ ] Остановленная **sandbox**-сессия перезапускается кнопкой «Запустить заново»: новая сессия создаётся с тем же счётом, тикером, таймфреймом и режимом `sandbox` (раньше — ошибка 422).
- [ ] Остановленная **real**-сессия перезапускается так же: режим не подменяется на «бумажный».
- [ ] Paper-сессия перезапускается как прежде, ставка комиссии переносится.
- [ ] После перезапуска открывается карточка **новой** сессии, а не прежней.
- [ ] Если по этому инструменту на этом счёте уже работает другая сессия — кнопка «Запустить заново» неактивна, а под шапкой видно, какая именно сессия мешает (номер, тикер, счёт).
- [ ] У сессии, созданной до появления обязательного таймфрейма, кнопка неактивна с объяснением «не сохранён таймфрейм» — а не запускается на чужом таймфрейме.
- [ ] Форма запуска сессии продолжает работать как раньше: paper без счёта, sandbox/real со счётом, конфликт подсвечивается заранее.

## S8.32 Удаление брокерского счёта с живыми сессиями (S8R, 2026-08-10)

- [ ] Счёт с сессией в статусе «Активна» удалить нельзя: показывается отказ, счёт остаётся в списке.
- [ ] То же для сессии на паузе и для приостановленной рестартом сервера.
- [ ] В тексте отказа перечислены мешающие сессии: номер, тикер и статус каждой, плюс подсказка «остановите их на странице Торговля».
- [ ] Модалка удаления остаётся открытой — текст отказа можно дочитать.
- [ ] Счёт только с остановленными (`stopped`) сессиями удаляется штатно.
- [ ] После отказа привязка сессий к счёту не изменилась: сессии продолжают отображаться на своём счёте, торговля не прервана.
- [ ] Счёт без сессий удаляется как раньше.

## S8.33 Курс доллара в виджете баланса (S8R, 2026-08-10)

- [ ] Переключатель RUB/USD пересчитывает баланс по курсу ЦБ РФ, а не по «примерно 90».
- [ ] Сумма в долларах совпадает с ручным расчётом «рубли ÷ курс ЦБ на сегодня» (курс — cbr.ru).
- [ ] Выбор валюты запоминается между перезагрузками страницы.
- [ ] Если ЦБ недоступен, но курс был получен раньше: под суммой видна дата, на которую курс установлен.
- [ ] Если курса нет вовсе: переключатель валюты **не показывается**, баланс в рублях отображается штатно, виджет не показывает ошибку.
- [ ] Повторное открытие дашборда в пределах нескольких часов не вызывает новых обращений к ЦБ (курс кэшируется).

## S8.34 Сборка production-образа фронтенда (S8R, 2026-08-10)

- [ ] `cd Develop/frontend && pnpm typecheck` — 0 ошибок (это `tsc -b`, а не `tsc --noEmit`).
- [ ] `pnpm build` проходит и создаёт `dist/` — ровно та команда, которой собирается frontend-образ.
- [ ] В CI на PR присутствуют шаги «Type check (tsc -b)» и «Production build» и оба зелёные.
- [ ] Намеренная ошибка типа в любом `.tsx` роняет `pnpm typecheck` (проверка, что гейт снова живой).

## S8.35 Повторный резолв FIGI по нерезолвящемуся тикеру (S8R, 2026-08-10)

- [ ] При работающей sandbox-сессии по тикеру, которого у брокера нет, запись `figi_unresolved` в логе появляется **один раз**, а не на каждую свечу.
- [ ] Спустя 15 минут попытка резолва повторяется: в логе снова `figi_unresolved` — либо `figi_synced`, если инструмент появился.
- [ ] Кратковременная недоступность брокера **не** отключает попытки на 15 минут: после восстановления связи FIGI подхватывается на ближайшем сигнале.
- [ ] Сделки по обычным тикерам по-прежнему получают настоящий FIGI — сверка с брокером расхождений не показывает.

## S8.36 Авто-сворачивание фоновых бэктестов (S7R, 2026-08-10)

- [ ] Завершённый фоновый бэктест исчезает из бейджа примерно через 30 секунд после готовности.
- [ ] Пока список бэктестов открыт, карточка не исчезает; отсчёт идёт после закрытия списка.
- [ ] Карточка с ошибкой остаётся до тех пор, пока её не убрать вручную (текст причины больше нигде не виден).
- [ ] Отменённый бэктест сворачивается наравне с успешным.
- [ ] После перезагрузки страницы давно завершённые карточки не возвращаются в бейдж.

## S8.37 Тариф брокерского счёта для paper-сессии (S8R, 2026-08-10)

- [ ] В форме запуска paper-сессии есть переключатель «Ставка вручную» / «Тариф брокерского счёта».
- [ ] При выборе тарифа появляется список счетов; для выбранного видна его ставка («Тариф счёта: 0,3 % с каждой стороны сделки»).
- [ ] Счёт с нулевым тарифом сопровождается предупреждением, что комиссия удерживаться не будет.
- [ ] Запуск с выбранным источником «тариф счёта», но без счёта, не проходит — поле подсвечено ошибкой.
- [ ] Запущенная сессия удерживает комиссию по тарифу счёта: сумма видна в колонке «Комиссия» сделки.
- [ ] Изменение тарифа счёта после запуска **не** меняет ставку уже работающей сессии.
- [ ] В режимах sandbox и real переключателя и поля комиссии нет — комиссию считает брокер.

## S8.38 Режим B — импорт стратегии из текстового шаблона (S5R, 2026-08-10)

- [ ] В шапке редактора стратегии есть кнопка «Режим B» рядом с «Генерировать».
- [ ] Кнопка открывает окно «Импорт стратегии из шаблона» с двумя кнопками копирования: пустой шаблон и заполненный пример.
- [ ] Пустой шаблон содержит семь секций (ИНДИКАТОРЫ … ФИЛЬТРЫ) и **не** содержит названия стратегии, инструментов и таймфрейма.
- [ ] «Проверить» на скопированном примере показывает список распознанных блоков без ошибок и предупреждений.
- [ ] «Применить» переносит блоки в редактор, после чего код помечается устаревшим («Блоки изменены — код устарел»).
- [ ] При пустом поле кнопка «Проверить» недоступна, «Применить» — недоступна до успешной проверки.

## S8.39 Регистрация закрыта после первого пользователя (S8R-AUDIT-033, 2026-09-24)

- [ ] На чистой БД `/login` сам уводит на `/setup`; после создания пользователя открывается приложение.
- [ ] На настроенной системе прямой переход на `/setup` уводит на `/login` (кратко виден индикатор загрузки, формы регистрации нет).
- [ ] На странице входа нет ссылки «Первый запуск? Создать аккаунт».
- [ ] Повторный `POST /api/v1/auth/setup` на настроенной системе → 403 «Регистрация закрыта…»; частые открытия `/login` (опрос `setup-status`) не приводят к 429 при входе.

## S8.40 Удаление стратегии, расхождение позиций, пределы риск-параметров (S8R-AUDIT-069/100, 025, 080, 2026-09-25)

- [ ] Удаление стратегии с работающей/приостановленной сессией: красное уведомление с текстом отказа и перечнем сессий (номер, тикер, статус), модалка удаления остаётся открытой, стратегия не удалена.
- [ ] Удаление стратегии без живых сессий проходит как раньше; повторное удаление той же стратегии (двойной клик) не даёт ошибки сервера.
- [ ] «Настройки уведомлений»: есть строка «Расхождение позиций с брокером»; каналы включаются и сохраняются.
- [ ] Конструктор: блок «Стоп-лосс» в режиме «%» не даёт ввести больше 50, в режиме «пункты» — до 100; сохранение стратегии со стоп-лоссом вне диапазона (через API) даёт понятный отказ «какой блок, какое поле» (S8R-AUDIT-080).

## S8.41 Short-стратегия в торговле (S8R-AUDIT-032, 2026-09-25)

- [ ] Paper-сессия стратегии с направлением «Short»: первая сделка в таблице сделок — «Short», вход по сигналу входа, выход по сигналу выхода; P&L сделки = (вход − выход) × штуки − комиссия.
- [ ] После закрытия прибыльного short бумажный баланс (капитал) вырос на P&L сделки, после убыточного — уменьшился; «Заблокировано» вернулось к 0.
- [ ] Long-стратегия: условие выхода срабатывает при отсутствии позиции → ордера и сделки нет, сессия не встаёт на паузу, уведомлений о Circuit Breaker нет.
...
| S8.28 Сверка с брокером и доучёт ордера | 11 | S8R-EXIT-ORDER-NOT-TRACKED, S8R-SANDBOX-POSITIONS-NOT-RECONCILED, S8R-TINVEST-SANDBOX-FLAKY-70001 |
| S8.29 Настоящий FIGI и фоновая сверка | 10 | S8R-SANDBOX-TRADES-FAKE-FIGI, S8R-RECONCILE-LOCK-DIVERGENCE, S8R-RECONCILE-NOT-PERIODIC |
| S8.30 Один инструмент — одна сессия на счёте | 11 | S8R-DUPLICATE-SESSION-PER-INSTRUMENT, S8R-RECONCILE-NO-ACCOUNT-DEDUP, S8R-PAPER-CONFLICT-BY-VERSION-NOT-TIMEFRAME |
| S8.31 «Запустить заново» для sandbox/real | 7 | S7R-SESSION-RERUN-PAYLOAD-BROKEN |
| S8.32 Удаление счёта с живыми сессиями | 7 | S8R-DELETE-ACCOUNT-ORPHANS-SESSIONS |
| S8.33 Курс доллара от ЦБ РФ | 6 | S9-MULTICURRENCY-CBR-RATE |
| S8.34 Сборка production-образа фронтенда | 4 | S8R-FRONTEND-BUILD-BROKEN |
| S8.35 Повторный резолв FIGI | 4 | S8R-FIGI-NO-NEGATIVE-CACHE |
| S8.36 Авто-сворачивание фоновых бэктестов | 5 | S7R-BG-BACKTEST-AUTOCOLLAPSE |
| S8.37 Тариф счёта для paper-сессии | 7 | S8R-PAPER-TARIFF-UNREACHABLE-FROM-UI |
| S8.38 Режим B — импорт из шаблона | 6 | S5R-BLOCKLY-MODE-B-MODAL, S5R-BLOCKLY-MODE-B-CHECK |
| S8.39 Регистрация закрыта после первого пользователя | 4 | S8R-AUDIT-033 |
| S8.40 Удаление стратегии, расхождение позиций, пределы риск-параметров | 4 | S8R-AUDIT-069, S8R-AUDIT-100, S8R-AUDIT-025, S8R-AUDIT-080 |
| S8.41 Short-стратегия в торговле | 3 | S8R-AUDIT-032 |
| **Итого S8 (новых пунктов)** | **278** | |
