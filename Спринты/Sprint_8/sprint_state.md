...
**Дата старта W8h (BUG-11 stale pending):** 2026-05-15 (новая sandbox-сделка id=15 застряла в pending без entry_price, T-Invest sandbox держит ордер в state='new' без встречной заявки; стратегия заблокирована через max_concurrent_positions. Тикет: `S8R-STALE-PENDING-TTL-CANCEL`.)
**Дата завершения W8h:** 2026-05-15 (1) `SessionRuntime.STALE_PENDING_CANCEL_THRESHOLD_SEC=1800` + `PERIODIC_RECOVERY_INTERVAL_SEC=60`. 2) ветка state='new' в `_recover_orphan_pending_trades` теперь проверяет возраст: > TTL → cancel_order + failed; cancel exception проглочен. 3) фоновый `_periodic_recovery_loop` стартует в `restore_all`, отменяется в `shutdown`. 4) **3 новых теста**: above-TTL cancel, below-TTL kept, cancel-fails-still-failed. Test runtime_orphan_recovery: **9 passed / 0 failed**. ФТ v2.13 → v2.14.
**Дата старта W8i (КОРНЕВОЙ БАГ ORDER_STATUS_MAP):** 2026-05-15 (диагностический эксперимент `scripts/diag_sandbox_orders.py` показал что T-Invest sandbox исполняет market-ордера мгновенно по last_price; корнем зависших pending-сделок был неверный ORDER_STATUS_MAP в `mapper.py:70-81` — значения сдвинуты с Sprint 1. Тикет: `S8R-PROTO-ENUM-ALIGNMENT`.)
**Дата завершения W8i:** 2026-05-15 Variant B: ORDER_STATUS_MAP исправлен (0-5 со значениями из официального `.proto`, удалены значения 6-9 как мусор); ACCOUNT_STATUS_MAP и ACCOUNT_TYPE_MAP дополнены пропущенными значениями. **11 новых тестов** с цитатами из `.proto`. Документация `tinvest_api_services.md` дополнена разделом «Приложение: enum-значения protobuf». Targeted regression: **358 passed / 0 failed**. ФТ v2.14 → v2.15.
**Дата старта W8j (фейковый PnL + CB-pause с активной позицией):** 2026-05-15 (после W8i sync ветка `filled` начала срабатывать → `response.price` (=`executed_order_price`) от T-Invest sandbox = total executed value, а не per-share. Реальный инцидент: trade #17 SBER session #3, entry_price=9763.20 вместо 325.44, pnl=-11715.84 фейк; `daily_loss_limit` поставил все сессии на паузу. Тикеты: `S8R-PRICE-SOURCE-FIX` + `S8R-CB-FORCE-CLOSE`.)
**Дата завершения W8j:** 2026-05-15 (1) `engine._submit_order_to_broker` синхронная filled-ветка + `close_position` sandbox/real-ветка: после `place_order` всегда `adapter.get_order_status` → `average_position_price` (per-share); response.price только как fallback при exception. (2) `circuit_breaker/engine._force_close_open_positions` (Вариант B) + интеграция в `_trigger()` — при срабатывании CB открытые позиции в затронутых сессиях принудительно закрываются; ошибки close swallow. (3) **4 новых regression-теста**: 3 entry/exit price source (W8j-1) + 1 CB force-close (W8j-2). (4) Скрипт `scripts/restore_w8j_inconsistent_data.py` восстановил trade #17 (entry=exit=325.44, pnl=0, volume_rub=9763.2), удалил cb_event #18, вернул session #3 в active. (5) Новый Gotcha 33 «T-Invest sandbox executed_order_price = total». sandbox_flow regression: **13 passed / 0 failed**.
**Дата старта W8k (stale-render SessionDashboard):** 2026-05-15 (фронтенд-баг: возврат назад из карты сессии A + клик на сессию B → видна карта A до завершения fetchSession. Тикет: `S8R-SESSION-DASHBOARD-STALE`.)
**Дата завершения W8k:** 2026-05-15 SessionDashboard: (1) `useTradingStore.setState({ activeSession: null })` перед fetchSession в useEffect, (2) guard `activeSession.id !== sessionId` → Loader. tsc 0 errors.
**Дата старта W8l (drag по вертикали + увеличенные trade-маркеры):** 2026-05-15 (фронтенд-баг: drag по Y в основной области графика не работает с Sprint 7 — регрессия фикса 2026-05-09. Trade-маркеры мелкие и накладываются. Тикеты: `S8R-CHART-VERTICAL-DRAG` + `S8R-MARKERS-2X-COLLISION`.)
...
**Дата старта W8q (Combobox с иконками как ActivePositionsWidget):** 2026-05-16 (заказчик: «не нравится два поля. Сделать иконку+тикер крупно, при клике dropdown с поиском и опциями с иконками, как в карточках активных позиций»). Тикет: `S8R-SPARKLINE-COMBOBOX-ICONS`.
**Дата завершения W8q:** 2026-05-16 Шапка переписана через Mantine `Combobox` low-level API. Trigger = `[TickerLogo] {ticker} [▾]` (стиль из `ActivePositionsWidget`). Dropdown = `Combobox.Search` + `Combobox.Options` с `{TickerLogo} {ticker} {name?}`. Подсказки: пустой ввод → `getRecentInstruments`, ≥1 символа → debounce 300мс → `searchInstruments`. Badge `24 ч` справа. W8o-тест адаптирован под Combobox UX (клик-trigger → опция). vitest: **10 passed / 0 failed**, tsc 0 errors.
**Дата старта W8r (dashboard instruments — unrealized PnL + sandbox-строки):** 2026-05-16 (заказчик: в таблице стратегий значения «Позиция»/«P&L» не соответствуют реальной открытой позиции; sandbox-сессия SBER #3 не отображается. Тикет: `S8R-DASHBOARD-INSTRUMENTS-UNREALIZED`.)
**Дата завершения W8r:** 2026-05-16 `get_instruments_summary` переписан: итерация по сессиям (не тикерам), все режимы (paper/sandbox/real) с position-payload, `current_price` из `OHLCVCache` (последняя свеча), `abs_pnl=(current−entry)×lots`, `pct_pnl` к cost-basis. Schema +`session_mode` поле. Frontend: ключ строки `${strategy}-${ticker}-${session_id}`, `SESSION_MODE_LABEL` (Paper/Sandbox/Real bыджи). 4 теста переписаны + новый кейс на two-sessions-same-ticker. Regression test_strategy: **158 passed / 0 failed**, tsc 0 errors.
**Дата старта W8s (колонка «Бэктест» — информативное содержимое):** 2026-05-16 (заказчик: «мне не очень понятно, что выводится в колонке "Бэктест". Не очень как-то информативно. Распиши и предложи». Тикет: `S8R-W8s-DASHBOARD-BACKTEST-COL`. Вариант A: усиление inline + цветной Badge для PF + WR + кол-во сделок + возраст; на уровне стратегии — `backtests_count` + `avg_profit_factor`.)
**Дата завершения W8s:** 2026-05-16 Backend: `InstrumentBacktest` +`total_trades`/`win_rate`, `StrategyResponse` +`backtests_count`/`avg_profit_factor`. `get_instruments_summary` возвращает 6-tuple. Frontend `DashboardPage`: ячейка «Бэктест» переработана — цветной `Badge` для PF (порог 1.0/1.5), inline `DD% · N сд. · WR% · возраст` с цветами по WR/DD-порогам, свёрнутая строка показывает `N тестов · ср. PF X.X`. Удалены устаревшие helper'ы. Regression: backend `test_strategy` **159 passed / 0 failed** (+1 новый тест на агрегаты), frontend `vitest DashboardPage` **9 passed / 0 failed**, tsc 0 errors. Миграции не требовались — поля `total_trades`/`win_rate` уже в `Backtest` модели. Ожидается подтверждение заказчика на commit + push.
**Hotfix W8s·1 (2026-05-16):** Визуальная сверка скриншота заказчика выявила два бага: (а) `WR 3226%`/`4828%`/`3333%` — `win_rate` в БД хранится как процент 0..100 (формула `wins/total*100` в `metrics.py`), frontend ошибочно умножал ещё раз → убрано `× 100` в `formatWinRate`, пороги цвета 0.4/0.55 → 40/55. (б) Средний PF стратегии = 172.8 при реальных 0.8–1.2 — один бэктест с PF=∞ ломал среднее. Заменено на медиану: `avg_profit_factor` → `median_profit_factor` во всём стеке (schemas/service/router/strategyApi.ts/DashboardPage); лейбл «ср. PF» → «медиан. PF». Тесты обновлены: `win_rate=Decimal("63.83")` (правильные единицы), новый сценарий с выбросом PF=5000 демонстрирует robustness медианы. Regression: backend 159/0, frontend vitest 9/0, tsc 0 errors.
**Дата старта W8t (email в профиле и notification settings):** 2026-05-16 (заказчик: «в моём профиле слетела привязка к email; на странице уведомлений тоже «email не подключён»». Тикет: `S8R-W8t-PROFILE-EMAIL-PERSIST`.)
**Дата завершения W8t:** 2026-05-16 Корень: LoginPage кладёт в authStore `{id:0, username}` без email/is_admin (token endpoint их не отдаёт); FirstRunWizardGate тянет `/users/me`, но ответ никуда не сохраняет. Решение: новый экшен `authStore.refreshUser()` — `GET /auth/me` с мержем в `state.user` (не throw'ит). LoginPage вызывает его после login(); FirstRunWizardGate переключен с `usersApi.getMe()` на `refreshUser()` — это исправляет уже залогиненных пользователей без re-login. `AuthUser` расширен `wizard_completed_at`. 2 регрессионных теста в `authStore.test.ts` (success + network error). Regression: tsc 0 errors, vitest auth+wizard 20/0 (18 → 20). БД не менялась — баг чисто frontend persistence. Ожидается визуальная проверка: `/settings → Profile` показывает email, `/settings → Notifications` строка «Email уведомления: mr.buzz@mail.ru».
**Дата старта W8u (CI lint fixes):** 2026-05-18 (заказчик: «постоянно приходят уведомления от GitHub об ошибках». 20+ последних CI runs красные подряд — падают lint-стадии. Тикет: `S8R-W8u-CI-LINT-FIXES`.)
**Дата завершения W8u:** 2026-05-18 Диагностика: backend ruff 8×E402 в `scripts/diag_sandbox_orders.py` (легитимные sys.path imports), frontend eslint 2 ошибки (unused `loading` SessionDashboard от W8k + unused `StrategyInstrumentSummary` DashboardPage — мой W8s residue). После открытия E402 всплыли ещё 4 ruff-ошибки: F821 SandboxBalanceResponse (forward-ref без TYPE_CHECKING-импорта), F401 PaperPortfolio (мой W8r residue), F401 os. Решение: `pyproject.toml` per-file-ignore `scripts/**/*.py = ["E402"]`; `app/broker/service.py` +TYPE_CHECKING import; `app/strategy/service.py` -PaperPortfolio; `scripts/diag_sandbox_orders.py` -os; `SessionDashboard.tsx` -loading; `DashboardPage.tsx` -StrategyInstrumentSummary. Локально: ruff/eslint/tsc 0 errors, pytest strategy+broker 299 passed, vitest 591 passed/86 файлов.
**Hotfix W8u·1 (2026-05-18):** После W8u-коммита (`8ded7e3`) frontend полностью зелёный, backend ruff зелёный, но открылась следующая CI-стадия — `mypy app/`. Накопилось 4 ошибки (раньше ruff блокировал доступ): (1) `notification/schemas.py:27` — invalid stand-alone `type: ignore` → превращён в обычный комментарий; (2) `strategy/service.py:283` — `date=bt.started_at` без None-guard в bt-only-tickers ветке (мой W8r residue) → `if bt.started_at is None: continue`; (3) `scheduler/service.py:343` — inline `True` в `and_()` ждущем `ColumnElement[bool]` → вынесено в `tf_filter` с `sa_true()`; (4) `trading/runtime.py:1266` — `cb_result` union `CheckResult | _CBResultPass` (W8b stub) → явная аннотация `cb_result: Any`. mypy: Success no issues 153 files; pytest tests/unit/: 957 passed.
**Hotfix W8u·2 (2026-05-18):** После W8u·1 CI прошёл ruff/mypy/unit tests, упал на **Coverage gate** (это `pytest tests/ --cov --cov-fail-under=80`, не unit/). Time-bomb тест `test_unrealized_pnl_uses_lot_size`: `lot_size_synced_at=datetime(2026, 5, 8)` hardcoded, `LOT_SIZE_TTL=7 дней`, сегодня 2026-05-18 → 10 дней > 7 → кэш истёк → `ensure_lot_size` шёл в T-Invest/ISS без mock → возвращал 1 → assert падал. Fix: `datetime.utcnow()`. Локально: `pytest tests/ --cov-fail-under=80` — 1606 passed, coverage **84.72%**. Других time-bomb в тестах не найдено (grep `lot_size_synced_at=datetime`).
**Дата старта W8v (торговые сессии пропадают + не открываются):** 2026-05-18 (заказчик: «обновляю /trading — данные пропадают, виджеты пустые, клик на сессию — крутится Loader навсегда»). Тикет: `S8R-W8v-WS-SNAPSHOT-MERGE`.
**Дата завершения W8v:** 2026-05-18 Корень: backend `ws_sessions._serialize_session` отдаёт WS-snapshot с `session_id` вместо `id` и без полей `started_at`/`current_pnl`/`initial_capital`. Frontend `applySnapshot` ПЕРЕЗАПИСЫВАЛ массив sessions узким payload через `setState({sessions: ...})` → id=undefined → URL `/sessions/undefined`, React key warning, «Invalid Date», «Нет позиции». Fix: applySnapshot переписан на **merge** — нормализация `session_id → id`, REST-поля сохраняются из existing, новые сессии добавляются с правильным id. +2 регрессионных теста (vitest 7 → 9). Также SQL восстановление трёх suspended-сессий (id 1, 2, 3) → status='active', `touch app/main.py` для restore_all() в lifespan. Заказчик подтвердил визуально. tsc 0 errors.
**Acceptance-fix BUG-20 (per-user favorites, 2026-06-03):** Заказчик зашёл под тестовым пользователем (без избранных), открыл график и нажал звёздочку — увидел чужие тикеры (`LKOH`, `SBER`) и чужие избранные таймфреймы. Корень: оба списка хранились в `localStorage` под глобальными ключами `user_favorites` / `favoriteTimeframes` (без user_id-префикса), `authStore.logout()` их не очищал. Fix: новый backend-модуль `app/user_favorites/` (модель `UserFavorite(user_id, kind, value)`, 3 REST endpoint'а), Alembic миграция `a7b3c9d5e1f2`, frontend `userFavoritesApi.ts` + Zustand `userFavoritesStore.ts` (не persist). FavoritesPanel и marketDataStore.favoriteTimeframes переключены на backend. `authStore.logout()` теперь reset()-ит store + удаляет legacy localStorage ключи. **Тесты**: backend 11 (включая cross-user isolation), frontend 5 (включая «не читает legacy localStorage»). Quality gates: pytest **1649 passed**, vitest **597 passed**, tsc clean. Тикет: `S8R-ACCEPTANCE-FIX-BUG-20`.

**Acceptance-fix BUG-22 (блочная стратегия не открывает сделку, 2026-06-05):** testuser1 запустил sandbox-сессию #5 (SBER 1m, strategy version 92), за ~сутки 0 сделок (`last_signal_at`=NULL, `live_trades` пусто). Вопрос «условия не довели до сделки?» — НЕТ, баг кодогенерации. Корень: live/sandbox-движок конвертирует `blocks_json` → sandbox-скрипт (`generated_code` = backtrader), но `_blocks_to_sandbox` строил имена переменных как `f"sma_{block_id}"` из сырого Blockly-id с символами, недопустимыми в Python-идентификаторах (`= % ^ ) [ | : ` ( `; id sv=92: `7LDZ=b6%^r%y)4oI[6|u`). Код не компилировался → `SyntaxError` в CodeSandbox → `process_candle`=None → runtime молча без сигнала на каждой свече. Воспроизведено на реальной sv=92: SMA(20)≈322 > 70 = True (должен `buy`); до фикса пустой output, после — `buy`. Fix: имена из монотонного индекса (`ind_sma_0`, …), block_id остаётся ключом `ind_vars`. **Тесты (TDD)**: новый `test_engine_blocks_to_sandbox.py` 6/6 (RED `SyntaxError` → GREEN), `test_trading` 179, полный pytest **1656**. Сопутствующее (не в scope): exit `condition_in_zone` не поддержан (кандидат BUG-23), SL/TP из management-блоков игнорируются, 1m-свечи остановились на 2026-06-04 10:19. Тикет: `S8R-ACCEPTANCE-FIX-BUG-22`.

**Acceptance-fix BUG-23 (единый интерпретатор стратегий + differential-сверка backtest⇄live, 2026-06-10):** продолжение находки #1 из BUG-22. Корень: ДВА переводчика блоков (бэктест → backtrader `code_generator`; live → урезанный `_blocks_to_sandbox`, понимал только `condition_compare`>число и SMA/EMA/RSI) расходились → live вёл себя не как протестированный бэктест (exit `condition_in_zone` у sv=92 молча не срабатывал). Решение (спека+план `docs/superpowers/.../2026-06-10-strategy-interpreter*`): единый интерпретатор `evaluate(ir,candles)` для live + differential-сверка сигналов в бэктесте. Реализовано 17 задач по TDD (subagent-driven, ревью spec→quality), 15 коммитов `a0a825c`…`50119f1` на ветке `s8r/bug-23-interpreter`. Новые модули: `indicators.py` (8 индикаторов, выверены к backtrader), `ir.py` (`parse_blocks`), `evaluator.py` (`evaluate`/`evaluate_series`), `backtest/parity.py`, `strategy/params.py`. Гейт live (sandbox/real, paper exempt): 409 `parity_no_backtest`/`parity_signals_diverged` + override + audit; миграция `b8c4d2e6f3a1`; UI бейдж + override. **Доказано backtest=live** для 7 форм (`test_signal_parity.py`, реальный backtrader⇄интерпретатор, 7/7). **Метрики**: backend pytest **1801 passed, 1 xfailed**, frontend vitest **618 passed**, tsc(app) чист по затронутым, pyright 0. Известное расхождение: Stochastic (Fast vs slow, xfail). **Новый баг (кандидат BUG-24):** `code_generator` → `bt.indicators.Stochastic(d_period=...)` (нужен `period_dfast`) → Stochastic-стратегии падают в бэктесте. **Поведение:** старт sandbox/real требует parity-checked бэктест и блокирует при расхождении сигналов. Статус: локально, не слито/не запушено. Тикет: `S8R-ACCEPTANCE-FIX-BUG-23`.

**Приёмка BUG-23 (2026-06-11):** Финальное whole-branch код-ревью — **APPROVED, блокеров нет** (1 Important + 3 Minor, не блокеры). **E2E под testuser1** (sandbox, рыночные часы): гейт `parity_no_backtest` (409, кнопка disabled) ✅; бейдж сверки в бэктесте #42 «расходится на 2 барах» + P&L-расхождение ✅; гейт `parity_signals_diverged` + override-чекбокс ✅; live sandbox #6 (SBER 1m) **сразу исполнила buy 31 лот @ 322** → BUG-22 окончательно закрыт, поток свечей стартует ✅. Расхождение #42 — не codegen-баг, а устаревший `generated_code` (exit-блок изменён без регенерации) — parity-гейт сработал по назначению. Сессия #6 оставлена активной для подтверждения долгого стопора SP-B.

**Фиксы приёмки (2026-06-11, в текущем спринте, TDD, код-ревью APPROVED):** **BUG-24 ✅** (`code_generator.py:220` `d_period`→`period_dfast` — Stochastic-стратегии больше не падают в бэктесте); **BUG-25 ✅** = SP-B (`multiplexer._run_stream` — watchdog idle-таймаута 180с через `wait_for(__anext__)` + gRPC keepalive → тихий обрыв стрима теперь форсирует reconnect, не висит вечно; enum-mismatch оказался ложной тревогой в SDK 0.2.0-beta117 — не трогали); **доводки гейта BUG-23 ✅** (`is not True` против обхода при `match=None` + версионность гейта зафиксирована тестом). Полный backend pytest: **1806 passed, 1 xfailed, 0 failed**. Тикеты: `S8R-ACCEPTANCE-BUG-24`, `S8R-ACCEPTANCE-SP-B`.

**Hotfix W8v·1 (GAZP в LaunchSessionModal, 2026-05-18):** Заказчик: «в выпадающем списке тикеров для запуска сессии есть только активные сессии — нет GAZP, который тестировался». Корень: `getRecentInstruments()` (localStorage) обновляется только при `startSession`, не при бэктесте. Fix: helper `collectSuggestedTickers(userStrategies)` объединяет recent + тикеры из `strategies[].instruments[].ticker`. Подписка на `useStrategyStore` через `userStrategies` (имя `strategies` занято под локальный Select формы — конфликт names поймал Vite oxc parser; пришлось переименовать в 6 точках). Применено в 4 местах (initial state + handleTickerSearch + opened-блок + useEffect для async strategy load). vitest LaunchSessionModal: 5 passed. Заказчик подтвердил: GAZP виден в списке. Тикет: `S8R-W8v-LAUNCH-TICKERS-FROM-BACKTESTS`.
