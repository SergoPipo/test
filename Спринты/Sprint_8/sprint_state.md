...
**Hotfix W8u·1 (2026-05-18):** После W8u-коммита (`8ded7e3`) frontend полностью зелёный, backend ruff зелёный, но открылась следующая CI-стадия — `mypy app/`. Накопилось 4 ошибки (раньше ruff блокировал доступ): (1) `notification/schemas.py:27` — invalid stand-alone `type: ignore` → превращён в обычный комментарий; (2) `strategy/service.py:283` — `date=bt.started_at` без None-guard в bt-only-tickers ветке (мой W8r residue) → `if bt.started_at is None: continue`; (3) `scheduler/service.py:343` — inline `True` в `and_()` ждущем `ColumnElement[bool]` → вынесено в `tf_filter` с `sa_true()`; (4) `trading/runtime.py:1266` — `cb_result` union `CheckResult | _CBResultPass` (W8b stub) → явная аннотация `cb_result: Any`. mypy: Success no issues 153 files; pytest tests/unit/ (то что прогоняет CI): 957 passed. Известный failed `tests/test_notification/test_telegram_positions.py::test_unrealized_pnl_uses_lot_size` — давний баг (lot_size mock = 1 вместо 10), падает и на baseline, **CI не запускает** (не в tests/unit/).
