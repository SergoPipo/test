...
## 2026-04-14 — Открытие Sprint_5_Review (внеплановое)

**Решение:** при подготовке к Sprint 6 обнаружен накопленный долг, который критически блокирует запуск S6. Вариант «перенести задачи в S6» отвергнут как тупиковый — S6 и так перегружен уведомлениями + recovery, а добавление туда CI cleanup + Live Runtime Loop + реальные позиции + E2E S4 fix + обкатки нового процесса работы DEV-субагентов превратит его в хаос, где повторится паттерн Sprint 5 (формальная реализация без runtime-интеграции, декларативные чеклисты без реальной проверки).

**Принято:** открыть **внеплановое промежуточное ревью** `Sprint_5_Review` между S5 и S6. Плановое `Sprint_6_Review` (после S5+S6, M3 full) остаётся на своём месте, это ревью — его промежуточный младший брат.

**Состав S5R (6 задач):**

| # | Задача | Источник | Приоритет | Документ |
|---|--------|----------|-----------|----------|
| S5R.1 | CI cleanup (ruff + eslint) | Новый, обнаружено 14.04 через Gmail MCP | 🔴 Блокер | `Sprint_5_Review/PENDING_S5R_ci_cleanup.md` |
| S5R.2 | Live Runtime Loop (SessionRuntime + timeframe) | Техдолг #4 High из S5, была задача 6.0 S6 | 🔴 Блокер | `Sprint_5_Review/PENDING_S5R_live_runtime_loop.md` |
| S5R.3 | Реальные позиции T-Invest | Техдолг #3 Medium из S5, была задача 6.9 S6 | 🟡 Важно | `Sprint_5_Review/PENDING_S5R_real_account_positions_operations.md` |
| S5R.4 | E2E S4 fix (30+ тестов) | Техдолг #2 Medium из S5 (переоценено в High) | 🔴 Блокер | `Sprint_5_Review/PENDING_S5R_e2e_s4_fix.md` |
| S5R.5 | Обкатка нового процесса работы DEV-субагентов | retro-s5 14.04 | 🔴 Критично | процессная, встраивается в S5R.1-4 |
| S5R.6 | Актуализация ФТ/ТЗ за S5 + S5R | правило проекта | 🟡 Обязательно | в финальном arch_review_s5r.md |

**Этап A (сегодня, documentation-only):**

- Создана папка `Спринты/Sprint_5_Review/` с полным набором документации:
  - `README.md` — контекст, причина созыва, состав задач
  - `backlog.md` — детальный backlog всех 6 задач с критериями готовности
  - `execution_order.md` — последовательность выполнения + раздел **Cross-DEV contracts** (11 контрактов C1-C11)
  - `PENDING_S5R_ci_cleanup.md` — детальный план задачи 1 (новый файл)
  - `PENDING_S5R_live_runtime_loop.md` — детальный план задачи 2 (перемещён из `Sprint_6/`, обновлены ссылки)
  - `PENDING_S5R_real_account_positions_operations.md` — детальный план задачи 3 (перемещён из `Sprint_6/`, обновлены ссылки)
  - `PENDING_S5R_e2e_s4_fix.md` — детальный план задачи 4 (новый файл)
- Удалены из `Sprint_6/` перемещённые PENDING файлы (теперь пусто в `Sprint_6/` — S6 ещё не спланирован в деталях, это нормально)
- **`Спринты/project_state.md`:**
  - Текущий спринт: «S5 завершён → **Sprint_5_Review** (внеплановое, 🔄 в процессе) → S6»
  - S5 статус переведён в ⚠️ завершён с замечаниями (был ✅)
  - Добавлена строка «Sprint 5 Review» в таблицу прогресса
  - Техдолг #2 переоценён с Medium на High, все техдолги #2-5 перенесены «Когда исправить» → `Sprint_5_Review`
  - Добавлен новый техдолг #5 — CI cleanup (High)
  - Раздел «Перенесённые задачи» заполнен 4 записями
  - Sprint_5_Review добавлен в таблицу «Промежуточные ревью»
- **`Документация по проекту/development_plan.md`:**
  - Добавлен **новый раздел** «SPRINT 5 REVIEW: Внеплановое промежуточное ревью» перед разделом S6, с таблицей 6 задач S5R.1-6 + S5R.R (финальное ревью)
  - **Откачены** задачи 6.0 (Live Runtime Loop) и 6.9 (реальные позиции T-Invest) из раздела S6 — они перенесены в S5R
  - Перенумерованы оставшиеся задачи S6: 6.9 E2E → 6.9, 6.10 Security → 6.10
  - Обновлены зависимости S6 — теперь они от «S5R» вместо «S5»
  - Упрощён критерий завершения S6 и диаграмма параллельности (убраны упоминания Live Runtime Loop и Real Positions — они завершены в S5R)
- **`Спринты/Sprint_5/sprint_state.md`:**
  - Статус переведён в ⚠️ завершён с замечаниями → открыто S5R
  - Добавлен большой блок «Причина открытия Sprint_5_Review (2026-04-14)» с описанием 5 слабостей и CI-проблемы
- **`Спринты/Sprint_5/changelog.md`:** эта запись

**Этап B (следующая сессия):** создание `preflight_checklist.md`, `prompt_DEV-*.md` по `Спринты/prompt_template.md` (11 обязательных секций), `prompt_ARCH_review.md`, заполнение деталей Cross-DEV contracts в `execution_order.md`.

**Этап C (работа агентов):** запуск DEV-1 (задачи S5R.1 + S5R.4), DEV-2 (задача S5R.2), DEV-3 (задача S5R.3) — по возможности параллельно после завершения CI cleanup. ARCH выполняет S5R.6 и S5R.R в финале.

**Ожидаемый эффект:**
- Зелёный CI на `moex-terminal/develop` после 11 дней падений
- Замкнутый Live Runtime Loop — live-торговля **реально работает**, не просто «класс существует»
- Реальные позиции T-Invest в разделе «Счёт»
- Playwright baseline зелёный, можно добавлять новые E2E без шума
- Первое практическое подтверждение, что `prompt_template.md` + Stack Gotchas + Integration Verification работают. Если что-то не работает — доработка до старта Sprint 6.
- Sprint 6 стартует чистый: только уведомления + recovery + graceful shutdown + UX + E2E + security, как и было в изначальном плане

**Plan file:** `.claude/plans/` — утверждённый план реорганизации.

---

## 2026-04-14 — Ретроспектива S5: правила работы DEV-субагентов + актуализация Develop/CLAUDE.md

**Контекст:** при подготовке к Sprint 6 провёл ретроспективу Sprint 5 на уровне **промпт-файлов** `prompt_DEV-N.md`. Цель — найти систематические слабости, которые привели к 41 замечанию ручного тестирования и техдолгу (задачи 6.0 Live Runtime Loop и 6.9 Реальные позиции в S6). Оценка по 7 критериям «хорошего промпта» (формулировка задачи, ссылки на ТЗ/ФТ, список существующих файлов, архитектурные ограничения, cross-DEV контракты, критерии готовности, формат отчёта) выявила 5 характерных слабостей:

1. **Нет «ловушек стека»** — DEV-агенты не знали про Pydantic v2 Decimal→строка (замечание 41), вырезанный `__import__` в CodeSandbox (замечания 33, 35), клонирование order в Backtrader `notify_order` (Bugfix SL/TP 13.04).
2. **Нет интеграционной верификации** — промпты требовали «реализовать класс», но не «доказать, что класс вызывается в runtime». Результат: `SignalProcessor.process_candle()` формально принят ARCH-ревью, но в production нигде не вызывается. Техдолг #4 High → задача 6.0 в S6.
3. **Разорванный контракт DEV-1 ↔ DEV-5** — DEV-5 ожидал `/broker/accounts/{id}/positions` от DEV-1, в промпте DEV-1 таких endpoints нет. Техдолг #3 → задача 6.9 в S6.
4. **Нет формата отчёта субагента** — субагенты возвращают результаты в свободной форме.
5. **Ссылки на ТЗ без цитирования** — `«ТЗ 5.4.2»` вместо дословной цитаты требования.

Грубая оценка: **~15% замечаний S5 можно было предотвратить** на уровне промптов (4 замечания + 2 техдолга). Остальные 85% — новые фичи от ручного тестирования и UX-мелочи, которые промпты контролировать не могут.

**Принято архитектурное решение:** зафиксировать правила работы DEV-субагентов в **4-х разных местах** по принципу разделения ответственности (каждое место со своей семантикой и областью видимости), а не всё в одном `CLAUDE.md`.

### Правки в 4-х местах

1. **`Develop/CLAUDE.md`** — актуализирован после 5 спринтов разработки + добавлена новая секция «⚠️ Stack Gotchas».
   - Раздел «Стек технологий»: убраны конкретные версии пакетов, добавлена отсылка к `backend/pyproject.toml` и `frontend/package.json`. Добавлена критическая оговорка про библиотеки, используемые в коде, но отсутствующие в `pyproject.toml` (`backtrader`, `ta-lib`, `apscheduler`, `python-telegram-bot`, `tinkoff-investments`) — ставятся через `setup_macos.sh` или отдельный pip install.
   - Раздел «Структура проекта» (ASCII-дерево): **backend** — добавлен модуль `tax/` (S5), уточнены описания `trading/`, `notification/` (email+in-app как заглушки), `broker/` (добавлен `moex_iss/`), помечен `scheduler/service.py` как пустая заглушка (ожидает S6). **Frontend** — добавлены `components/blockly/`, `components/common/`, `services/`, удалён несуществующий `components/notifications/`, обновлены описания `trading/` и `account/`.
   - Новая секция «⚠️ Stack Gotchas» после раздела «Конвенции»: 5 ловушек с форматом симптом / корневая причина / правило обхода / ретро-ссылка на замечание S5.
     - Gotcha 1 — Pydantic v2 + Decimal → строка в JSON (ретро: замечание 41)
     - Gotcha 2 — CodeSandbox: `__import__` вырезан (ретро: замечания 33, 35)
     - Gotcha 3 — Backtrader: `notify_order` клонирует ордер (ретро: Bugfix SL/TP 13.04)
     - Gotcha 4 — T-Invest gRPC streaming: persistent connection обязателен
     - Gotcha 5 — Circuit Breaker: обязательный `asyncio.Lock`
   - Политика пополнения: каждое ARCH-ревью после спринта проверяет новые ловушки. Политика выноса: если секция превысит ~80 строк — вынести в `Develop/STACK_GOTCHAS.md`.

2. **`Спринты/prompt_template.md`** (новый файл) — базовый шаблон для всех будущих `prompt_DEV-N.md`. Содержит **11 обязательных секций**:
   - Frontmatter + Роль + Предварительная проверка
   - **⚠️ Обязательное чтение** — явные ссылки на `Develop/CLAUDE.md` секцию Stack Gotchas, `Sprint_N/execution_order.md` раздел Cross-DEV contracts, `ui_spec_sN.md` для frontend. Требование цитировать фрагменты ТЗ/ФТ дословно (не просто «ТЗ 5.4.2»).
   - Рабочая директория + Контекст существующего кода + Задачи + Тесты
   - **⚠️ Integration Verification Checklist** — grep-правила: `grep -rn "<ClassName>(" app/` для инстанциирования, `grep -rn "\.method(" app/` для вызова, проверка listener'ов на event_bus / stream_manager / scheduler / main.py lifespan, требование явной пометки `⚠️ NOT CONNECTED` в случае разрыва runtime-цикла (блокирующая ошибка для приёмки).
   - **⚠️ Формат отчёта (МАНДАТНЫЙ)** — структурированная сводка до 400 слов по 8 секциям: реализовано, файлы, тесты, integration points (с явной пометкой NOT CONNECTED при разрыве), контракты, проблемы, применённые Stack Gotchas, новые Stack Gotchas. Явное требование **НЕ возвращать** полный код и лог tool-вызовов.
   - Alembic-миграция (если применимо) + Чеклист перед сдачей.

3. **`Спринты/Sprint_N/execution_order.md`** — правило per-sprint (применяется с S6). Новый раздел «Cross-DEV contracts» с таблицей «Поставщик | Потребитель | Контракт | Формат». Каждый DEV обязан в отчёте явно подтвердить контракты, в которых он — поставщик или потребитель. Несоответствие = блокирующая ошибка. Структура зафиксирована в `prompt_template.md` и корневом `CLAUDE.md`, физические правки будут сделаны при создании `Sprint_6/execution_order.md`.

4. **Корневой `CLAUDE.md`** — в раздел «Процесс разработки» добавлен новый подраздел «Работа с DEV-субагентами в спринтах» (~20 строк). Мета-правила: обязательное использование шаблона, обязательное чтение Stack Gotchas, цитирование ТЗ, Cross-DEV контракты в execution_order, integration verification через grep, формат отчёта.

### Ожидаемый эффект для S6

- Сокращение повторяющихся багов от «ловушек стека»: замечания типа 33, 35, 41 и Bugfix SL/TP больше не должны появляться — правила зафиксированы в Stack Gotchas, DEV-субагент обязан их прочитать.
- Ликвидация gap'ов runtime-интеграции: Integration Verification Checklist требует grep-доказательства, что новый класс вызывается в production-коде. Нельзя будет сдать класс «просто реализованным» без подключения к runtime.
- Устранение разрывов контрактов между DEV: таблица Cross-DEV contracts в `execution_order.md` фиксирует контракты до запуска субагентов.
- Предсказуемый формат отчёта: оркестратор быстро видит, что сделано, что NOT CONNECTED, какие контракты доступны другим DEV.

### Файлы

- **Изменены:**
  - `Develop/CLAUDE.md` (стек + структура + Stack Gotchas)
  - `CLAUDE.md` (корневой) — новый подраздел «Работа с DEV-субагентами в спринтах»
- **Новые:**
  - `Спринты/prompt_template.md` — шаблон промпта DEV
- **План в `.claude/plans/`:** `effervescent-rolling-sky.md` — утверждённый план, описывает архитектуру решения «4 разных места» с обоснованием

### Применимо к следующему спринту

При создании `Sprint_6/execution_order.md` и `Sprint_6/prompt_DEV-*.md` — обязательное использование `prompt_template.md` + заполнение раздела «Cross-DEV contracts».
