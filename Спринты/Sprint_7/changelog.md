...
После S7R-PNL-DUAL-PCT (правки этапа A) юзер уточнил:
1. Формат должен быть со слэшем «поз. / кап.», а я делал через `·`.
2. Подписи «к позиции» / «к капиталу» слишком длинные.

### Реализовано

Везде (бот, `PositionsTable`, `SessionCard`):
- Разделитель `·` → `/`.
- «к позиции» → **«поз.»**, «к капиталу» → **«кап.»** (стандартные русские сокращения с точкой).

Итог:
```
−380.50 ₽ (−6.78% поз. / −0.38% кап.)
```

### Тесты

- `test_pnl_shows_both_percents` обновлён: ассерты на `9.45% поз.`, `0.01% кап.`, разделитель ` / `.
- 13/13 telegram, 36/36 trading vitest, tsc 0.
...
Пользователь после привязки бота через `/start` видел в Telegram «✅ Telegram привязан», но карточка в `Настройки → Уведомления` продолжала показывать алёрт с токеном, пока не истечёт 5-минутный таймер. UX-баг: фронт ничего не знал об операции, происходящей вне React (Telegram → backend webhook → БД).

### Реализовано

`frontend/src/pages/NotificationSettingsPage.tsx`:
- Добавлен `useEffect` с polling статуса `/telegram/status` каждые 3 секунды, активный пока `linkToken` есть и `telegramLinked === false`.
- При `linked: true` — обновляет `telegramLinked`/`telegramChatId`, чистит `linkToken`/`botUsername`/`linkTimer`. UI мгновенно переключается на «Привязан».
- Транзиентные ошибки (сеть/503) проглатываются — следующий тик попробует снова.
- `clearInterval` в cleanup — нет утечки таймера, при размонтировании или после успеха polling останавливается.

### Почему polling, а не WebSocket

WS-уведомления — overkill: событие срабатывает один раз за всю сессию пользователя, нагрузка ничтожна (≤100 запросов за 5 мин таймера, и только во время активной привязки). Polling не требует новой инфраструктуры на backend.

### Результат

После /start в боте → карточка в терминале мгновенно (≤3 сек) меняется с «Отправьте /start <code>» на «✅ Привязан». Старый путь (вручную перезагрузить страницу) больше не нужен.
