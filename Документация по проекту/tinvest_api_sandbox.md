...
- **Лимитные ордера**: не исполняются при отсутствии цены (last_price не пересёк указанную)
- **Рыночные ордера**: не исполняются при низкой ликвидности (нет сделок на бирже по инструменту в момент запроса)
- **Все неисполненные ордера удаляются автоматически в конце торговой сессии** (по официальному FAQ T-Invest)

> ⚠ **Sprint 8 W8i (2026-05-15)**: до W8i sandbox-сделки в нашей системе застревали в pending без `entry_price` — **не из-за поведения T-Invest**, а из-за бага в нашем `ORDER_STATUS_MAP` (`mapper.py`): T-Invest возвращал `EXECUTION_REPORT_STATUS_FILL=1`, наш код интерпретировал как `"new"`. Полная таблица актуальных enum-значений: [tinvest_api_services.md § Приложение](tinvest_api_services.md#приложение-enum-значения-protobuf-актуально-2026-05-15-после-w8i).
