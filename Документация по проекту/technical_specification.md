...
**Версия:** 1.0  
**Дата:** 2026-03-23  
**Основание:** Функциональные требования v2.0 от 2026-03-23  
**Статус:** Проект для ревью
...
**Ответственный: DevOps + все специалисты (ревью)**
...
**Ответственный: Все специалисты (определяет архитектор)**
...
**Ответственный: Специалист по БД**
...
**Ответственный: Backend-разработчик (определяет), Frontend-разработчик (ревью)**
...
**Ответственный: Специалист по безопасности (реализация) + Backend-разработчик (интеграция)**
...
**Ответственный: AI/ML-специалист (AI-часть) + Backend-разработчик (парсер, генератор)**
...
**Ответственный: Backend-разработчик (core) + DevOps (Telegram bot deploy)**

**Архитектура:**
- `NotificationService` слушает event bus и маршрутизирует события по каналам.
- Каналы: `TelegramNotifier`, `EmailNotifier`, `InAppNotifier`.
- Маршрутизация на основе `user_notification_settings`.

**TelegramNotifier:**
- Библиотека: `python-telegram-bot` в webhook-режиме (endpoint `/api/v1/telegram/webhook`).
- Верификация webhook через secret token.
- Форматирование сообщений по шаблонам из Приложения B (HTML parse mode).
- Inline-кнопки: "График" (deep link), "Закрыть" (callback query с confirmation).
- Обработка команд: `/start`, `/positions`, `/status`, `/help`, `/close`, `/closeall`, `/balance`.

**EmailNotifier:**
- SMTP через `aiosmtplib`.
- Используется только для критических событий (circuit breaker, потеря связи, дневная статистика).
- Настраивается в `.env`: `SMTP_HOST`, `SMTP_PORT`, `SMTP_USER`, `SMTP_PASS`.

**InAppNotifier:**
- Создаёт запись в `notifications`.
- Пушит через WebSocket канал `notifications`.

### 5.8 Circuit Breaker

**Ответственный: Специалист по безопасности (логика) + Backend-разработчик (интеграция)**
...
**Ответственный: AI/ML-специалист**
...
**Ответственный: Frontend/UI-разработчик**
...
**Маршрут:** `/strategies/:id/edit`

**Layout:** Три панели:
1. **Центр (60%):** Blockly workspace — полноэкранный drag-and-drop редактор с кастомными блоками.
2. **Справа (30%):** AI-чат (sidebar, collapsible). Поле ввода, история сообщений, кнопка "Применить к блокам".
3. **Снизу (табы):** "Код" (Monaco read-only с подсветкой Python), "Описание" (textarea с markdown).

**Blockly-конфигурация:**

Кастомные категории блоков в toolbox:
- **Индикаторы:** EMA, SMA, RSI, MACD, Bollinger, ATR, Stochastic, Volume MA. Каждый — блок с dropdown для параметров (период, source).
- **Условия:** Crossover, CrossUnder, GreaterThan, LessThan, InZone.
- **Логика:** AND, OR, NOT.
- **Сигналы:** EntryLong, ExitLong. Имеют socket для подключения условий.
- **Фильтры:** TimeFilter (start/end time), DayOfWeekFilter.
- **Управление капиталом:** PositionSizer (% или сумма), MaxPositions.
- **Стоп/Тейк:** StopLoss, TakeProfit (% или фикс.).

Blockly workspace генерирует JSON при каждом изменении -> отправляется на `/api/v1/strategies/{id}/generate-code` -> отображается во вкладке "Код".

**Режим B:**
- Кнопка "Вставить из внешнего источника" над workspace.
...
**Tab "Обзор":**
- Сетка метрик (карточки): Чистая прибыль (абс. + %), Кол-во сделок, Win Rate, Profit Factor, Max Drawdown, Sharpe, Средняя прибыль/убыток, Средняя длительность.
- Benchmark-сравнение: стратегия vs Buy & Hold vs IMOEX (горизонтальная bar chart).

**Tab "График":**
- Equity curve (LineSeries, зелёная) + drawdown (AreaSeries, красная, инвертированная).
- Benchmark lines (Buy & Hold — серая, IMOEX — синяя).
- Свечной график инструмента с маркерами входа/выхода.
- Два графика — split-view вертикально.

**Tab "Сделки":**
- `TanStack Table`: дата входа, дата выхода, цена, объём, P&L (зелёный/красный), длительность.
...
**Ответственный: DevOps-специалист**
...
3. Для каждой активной trading session: сохранить состояние в БД (status → 'suspended'), отменить активные подписки.
4. Дождаться завершения текущих ордеров (timeout 30s).
5. Закрыть gRPC-каналы, WebSocket-соединения.
6. Отправить уведомление "Система остановлена" в Telegram.
7. Закрыть БД-соединения.
...
**Ответственный: Все специалисты (каждый пишет тесты для своего модуля)**
...
**Все специалисты параллельно.**

| Задача | Ответственный |
|---|---|
| Инициализация проекта: backend (FastAPI + SQLAlchemy), frontend (React + Vite + Mantine) | DevOps + Backend + Frontend |
| Скрипты установки/запуска (install.sh, start.sh, .env) | DevOps |
| БД: все модели SQLAlchemy, Alembic init + первая миграция | DB-специалист |
| Auth module: регистрация, логин, JWT, refresh, brute-force protection | Безопасность |
| Frontend shell: layout, sidebar, header, routing, dark theme | Frontend |
| Health endpoint | Backend |

**Результат:** Можно залогиниться, увидеть пустой дашборд, health endpoint работает.

### Спринт 2 (2 недели): Данные и графики

| Задача | Ответственный |
|---|---|
| Broker adapter: T-Invest — подключение, get_accounts, get_balance, get_historical_candles | Backend |
| MOEX ISS client (fallback для candles + calendar) | Backend |
| Market Data Service: кеш OHLCV, загрузка, валидация | Backend + DB |
| Frontend: Charts (TradingView Lightweight Charts), таймфрейм selector, volume | Frontend |
| Frontend: Account/Broker settings, добавление API-ключа | Frontend |
| Crypto service (AES-256-GCM) для API-ключей | Безопасность |
| MOEX calendar service | Backend |

**Результат:** Подключён брокер, можно просматривать графики с историческими данными.

### Спринт 3 (2 недели): Стратегии и блочный редактор

| Задача | Ответственный |
|---|---|
| Strategy CRUD API + модели | Backend |
| Blockly: кастомные блоки (индикаторы, условия, сигналы, фильтры) | Frontend |
| Code generator: блоки → Python/Backtrader | Backend |
| Template parser (режим B) | Backend |
| Sandbox: AST-анализ + RestrictedPython | Безопасность |
| Frontend: Strategy Editor page (Blockly + Code tab + Description tab) | Frontend |

**Результат:** Можно создать стратегию в блочном редакторе или вставить из шаблона, увидеть сгенерированный код.

### Спринт 4 (2 недели): AI-интеграция + бэктестинг

| Задача | Ответственный |
|---|---|
| AI Service: pluggable providers (Claude, OpenAI), промпты, context compression | AI/ML-специалист |
| AI chat API + SSE streaming | AI/ML + Backend |
| Frontend: AI sidebar chat в Strategy Editor | Frontend |
| Backtest Engine: Backtrader wrapper, метрики, equity curve | Backend |
| Backtest API + progress через WebSocket | Backend |
| Frontend: Backtest Results (3 вкладки: обзор, график, сделки) | Frontend |
| Frontend: запуск бэктеста (форма параметров) | Frontend |

**Результат:** Полный цикл: описание стратегии (с AI или без) → блоки → код → бэктест → просмотр результатов.

### Спринт 5 (2 недели): Торговля

| Задача | Ответственный |
|---|---|
| Trading Engine: session manager, signal processor, order manager, position tracker | Backend |
| Paper Trading Engine | Backend |
| Circuit Breaker (все виды лимитов) | Backend + Безопасность |
| Broker adapter: place_order, cancel_order, subscribe_candles (streaming) | Backend |
| Rate limiter для брокерского API | Backend |
| Frontend: Trading Panel (карточки сессий, launch form, позиции) | Frontend |
| Frontend: Real-time обновления графиков через WS | Frontend |
| Индикаторы на графике | Frontend |

**Результат:** Можно запустить Paper Trading, видеть сделки в реальном времени. Circuit breaker работает.

### Спринт 6 (2 недели): Уведомления + восстановление

| Задача | Ответственный |
|---|---|
| Telegram bot: привязка, уведомления, команды (/positions, /status, /help) | Backend |
| Email notifications (SMTP) | Backend |
| In-app notifications (WebSocket) + Notification Center | Backend + Frontend |
| Event bus: все события | Backend |
| Восстановление после перезапуска | Backend |
| Graceful shutdown | DevOps + Backend |
| CSRF protection | Безопасность |
| Rate limiting (API) | Безопасность |

**Результат:** Полная система уведомлений. Система восстанавливается после перезапуска.

### Спринт 7 (2 недели): Полировка + Should-фичи

| Задача | Ответственный |
|---|---|
| Версионирование стратегий (Should) | Backend + Frontend |
| Grid search optimization (Should) | Backend + Frontend |
| Экспорт CSV/PDF (Should) | Backend + Frontend |
| Telegram-команды /close, /closeall, /balance (Should) | Backend |
| Ценовые алерты (Should) | Backend + Frontend |
| Инструменты рисования на графике (Should) | Frontend |
| Дашборд: виджет баланса, health indicator | Frontend |
| Дисклеймер (first-run wizard) | Frontend + Backend |
| Backup/restore | DevOps + Backend |

**Результат:** Phase 1 feature-complete.

### Спринт 8 (2 недели): Тестирование + стабилизация

| Задача | Ответственный |
|---|---|
| Unit-тесты: coverage >= 80% | Все |
| Интеграционные тесты (mock broker, sandbox) | Backend + Безопасность |
| E2E-тесты (Playwright) | Frontend |
| Performance testing: latency целевые значения | Backend |
| Security audit: проверка всех криптографических операций | Безопасность |
| Bug fixing | Все |
| Документация: README, deployment guide | DevOps |

**Результат:** Production-ready Phase 1.
...
Спринт 2 (данные + графики)     ← зависит от auth и DB
    ↓
Спринт 3 (стратегии)            ← зависит от market data (для тестов)
    ↓
Спринт 4 (AI + бэктест)         ← зависит от стратегий и market data
    ↓
Спринт 5 (торговля)             ← зависит от бэктест engine и broker adapter
    ↓
Спринт 6 (уведомления)          ← зависит от торговли (события)
    ↓
Спринт 7 (полировка)            ← параллельно можно начать после спринта 5
    ↓
Спринт 8 (тесты)                ← после feature freeze
```
...
| Модуль / Задача | DB | Backend | Frontend | DevOps | AI/ML | Security |
|---|---|---|---|---|---|---|
| Схема БД, миграции, индексы | **P** | R | | | | R |
| FastAPI app, middleware | | **P** | | R | | R |
| Auth module | | R | | | | **P** |
| Strategy Engine (parser, codegen) | | **P** | | | R | |
| Blockly editor | | | **P** | | | |
| AI Service (providers, prompts) | | R | | | **P** | |
| AI chat frontend | | | **P** | | R | |
| Backtest Engine | | **P** | | | | |
| Trading Engine | | **P** | | | | R |
| Broker Adapter (T-Invest) | | **P** | | | | |
| Market Data Service | R | **P** | | | | |
| Charts (TradingView) | | | **P** | | | |
| Notification Service | | **P** | R | | | |
| Telegram Bot | | **P** | | R | | R |
| Circuit Breaker | | R | | | | **P** |
| Code Sandbox | | R | | | | **P** |
| Crypto (AES, Argon2, JWT) | | | | | | **P** |
| CSRF, Rate Limiting, Headers | | R | | | | **P** |
| Скрипты deploy, CI/CD | | | | **P** | | |
| Backup, Logs, Health | | R | | **P** | | |
| Dashboard, Trading Panel, Settings | | | **P** | | | |
| E2E-тесты | | | **P** | | | |
| Unit/Integration-тесты | R | **P** | R | | R | **P** |
