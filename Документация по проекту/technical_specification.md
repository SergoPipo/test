...
При превышении: HTTP 429 с заголовком `Retry-After`. Заголовок открыт для JS через CORS `expose_headers` (S8R-AUDIT-039).
