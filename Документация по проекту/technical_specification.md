...
| **§5 Бэктест: таймаут данных** | Выборка свечей ограничена `BACKTEST_DATA_TIMEOUT_SEC` (новый параметр `app/config.py`, по умолчанию **180 с**). Исчерпание → `BacktestDataTimeoutError` с русским текстом → job `status='error'`, запись `backtests` → `status='failed'` + `error_message` с той же причиной. Запись терминального статуса обязана иметь fallback на свежей сессии из фабрики: отмена по `asyncio.wait_for` отравляет текущую `AsyncSession` (`gotcha-38`). | S9-BACKTEST-DATA-TIMEOUT |
